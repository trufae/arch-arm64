...
static ExprId GetILOperandPreOrPostIndex(LowLevelILFunction& il, InstructionOperand& operand)
{
	...
	if (operand.reg[1] == REG_NONE)
	{
		...
		return ILSETREG_O(operand,
		                  ILADDREG_O(operand, il.Const(REGSZ_O(operand), IMM_O(operand))));
	}
	else
	{
		// ..., [Xn], <Xm>
		return ILSETREG_O(operand,
		                  ILADDREG_O(operand, il.Register(8, operand.reg[1])));
	}
}
...
static ExprId GetILOperandEffectiveAddress(LowLevelILFunction& il, InstructionOperand& operand, size_t addrSize, OperandClass oclass, size_t extra_offset)
{
	...
	case MEM_EXTENDED:
		...
		else if (operand.shiftType == ShiftType_LSL)
		{
			if (extra_offset)
			{
				addr = il.Add(addrSize, ILREG_O(operand), il.Add(addrSize, il.ShiftLeft(addrSize, il.Const(addrSize, operand.immediate), il.Const(0, operand.shiftValue)), il.Const(addrSize, extra_offset)));
			}
			else
			{
				addr = il.Add(addrSize, ILREG_O(operand), il.ShiftLeft(addrSize, il.Const(addrSize, operand.immediate), il.Const(0, operand.shiftValue)));
			}
		}
		else
		{
    ...
    int load_store_sz)
{
	...
	else  //store
	{
		...
		case MEM_POST_IDX:
			...
			break;
		case MEM_EXTENDED:
			il.AddInstruction(il.Operand(1, il.Store(load_store_sz, il.Add(REGSZ_O(operand2), il.Register(REGSZ_O(operand2), operand2.reg[0]), GetShiftedRegister(il, operand2, 1, REGSZ_O(operand2))), ILREG_O(operand1))));
			break;
		default:
			il.AddInstruction(il.Unimplemented());
			break;
    ...
    InstructionOperand& operand2)
{
	...
	if (load)
	{
		...
		case MEM_POST_IDX:
			...
			break;
		case MEM_EXTENDED:
			tmp = il.Operand(1, il.Load(size, il.Add(REGSZ_O(operand2), ILREG_O(operand2), GetShiftedRegister(il, operand2, 1, REGSZ_O(operand2)))));
	...
	else  //store
	{
		...
		case MEM_POST_IDX:
			...
			break;
		case MEM_EXTENDED:
			il.AddInstruction(il.Operand(1, il.Store(size, il.Add(REGSZ_O(operand2), il.Register(REGSZ_O(operand2), operand2.reg[0]), GetShiftedRegister(il, operand2, 1, REGSZ_O(operand2))), valToStore)));
			break;
		default:
			il.AddInstruction(il.Unimplemented());
			break;
...
bool GetLowLevelILForInstruction(Architecture* arch, uint64_t addr, LowLevelILFunction& il, Instruction& instr, size_t addrSize)
{
	...
	case ARM64_ADRP:
		...
		break;
	case ARM64_ASR:
		il.AddInstruction(ILSETREG_O(operand1, il.ArithShiftRight(REGSZ_O(operand2), ILREG_O(operand2), ReadILOperand(il, operand3, REGSZ_O(operand2)))));
		break;
	case ARM64_AESD:
		il.AddInstruction(il.Intrinsic({RegisterOrFlag::Register(REG_O(operand1))}, ARM64_INTRIN_AESD, {ILREG_O(operand1), ILREG_O(operand2)}));
		break;
	case ARM64_AESE:
		il.AddInstruction(il.Intrinsic({RegisterOrFlag::Register(REG_O(operand1))}, ARM64_INTRIN_AESE, {ILREG_O(operand1), ILREG_O(operand2)}));
		break;
	case ARM64_BTI:
	...
	case ARM64_CASL:
		GenIfElse(il,
		          il.CompareEqual(REGSZ_O(operand1), ILREG_O(operand1), il.Load(REGSZ_O(operand1), ILREG_O(operand3))),
	...
	case ARM64_CASLH:
		GenIfElse(il,
		          il.CompareEqual(REGSZ_O(operand1), ExtractRegister(il, operand1, 0, 2, false, 2), il.Load(2, ILREG_O(operand3))),
	...
	case ARM64_CASLB:
		GenIfElse(il,
		          il.CompareEqual(REGSZ_O(operand1), ExtractRegister(il, operand1, 0, 1, false, 1), il.Load(1, ILREG_O(operand3))),
		          il.Store(1, ILREG_O(operand3), ExtractRegister(il, operand2, 0, 1, false, 1)),
		          0);
		break;
	case ARM64_CBNZ:
		ConditionalJump(arch, il, il.CompareNotEqual(REGSZ_O(operand1), ILREG_O(operand1), il.Const(REGSZ_O(operand1), 0)), addrSize, IMM_O(operand2), addr + 4);
		return false;
	case ARM64_CBZ:
		ConditionalJump(arch, il, il.CompareEqual(REGSZ_O(operand1), ILREG_O(operand1), il.Const(REGSZ_O(operand1), 0)), addrSize, IMM_O(operand2), addr + 4);
		return false;
	case ARM64_CMN:
		il.AddInstruction(il.Add(REGSZ_O(operand1),
		                         ILREG_O(operand1),
	...
	case ARM64_CSEL:
	case ARM64_FCSEL:
		GenIfElse(il, GetCondition(il, operand4.cond), ILSETREG_O(operand1, ILREG_O(operand2)), ILSETREG_O(operand1, ILREG_O(operand3)));
		break;
	case ARM64_CSINC:
		GenIfElse(il, GetCondition(il, operand4.cond), ILSETREG_O(operand1, ILREG_O(operand2)), ILSETREG_O(operand1, ILADDREG_O(operand3, il.Const(REGSZ_O(operand1), 1))));
		break;
	case ARM64_CSINV:
		GenIfElse(il, GetCondition(il, operand4.cond), ILSETREG_O(operand1, ILREG_O(operand2)), ILSETREG_O(operand1, il.Not(REGSZ_O(operand1), ILREG_O(operand3))));
		break;
	case ARM64_CSNEG:
		GenIfElse(il, GetCondition(il, operand4.cond), ILSETREG_O(operand1, ILREG_O(operand2)), ILSETREG_O(operand1, il.Neg(REGSZ_O(operand1), ILREG_O(operand3))));
		break;
	case ARM64_CSET:
		GenIfElse(il, GetCondition(il, operand2.cond), ILSETREG_O(operand1, il.Const(REGSZ_O(operand1), 1)), ILSETREG_O(operand1, il.Const(REGSZ_O(operand1), 0)));
		break;
	case ARM64_CSETM:
		GenIfElse(il, GetCondition(il, operand2.cond), ILSETREG_O(operand1, il.Const(REGSZ_O(operand1), -1)), ILSETREG_O(operand1, il.Const(REGSZ_O(operand1), 0)));
		break;
	case ARM64_CINC:
		GenIfElse(il, GetCondition(il, operand3.cond), ILSETREG_O(operand1, ILADDREG_O(operand2, il.Const(REGSZ_O(operand1), 1))), ILSETREG_O(operand1, ILREG_O(operand2)));
		break;
	case ARM64_CINV:
		GenIfElse(il, GetCondition(il, operand3.cond), ILSETREG_O(operand1, il.Not(REGSZ_O(operand1), ILREG_O(operand2))), ILSETREG_O(operand1, ILREG_O(operand2)));
		break;
	case ARM64_CNEG:
		GenIfElse(il, GetCondition(il, operand3.cond), ILSETREG_O(operand1, il.Neg(REGSZ_O(operand1), ILREG_O(operand2))), ILSETREG_O(operand1, ILREG_O(operand2)));
		break;
	case ARM64_CLZ:
	...
	case ARM64_EXTR:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.LogicalShiftRight(REGSZ_O(operand1) * 2,
		                                                  il.Or(REGSZ_O(operand1) * 2,
		                                                        il.ShiftLeft(REGSZ_O(operand1) * 2, ILREG_O(operand2), il.Const(1, REGSZ_O(operand1) * 8)),
		                                                        ILREG_O(operand3)),
	...
	case ARM64_FDIV:
		...
		case ENC_FDIV_D_FLOATDP2:
			il.AddInstruction(ILSETREG_O(operand1,
			                             il.FloatDiv(REGSZ_O(operand1), ILREG_O(operand2), ILREG_O(operand3))));
			break;
		default:
			il.AddInstruction(il.Unimplemented());
		}
	...
	case ARM64_FMOV:
		...
		case ENC_FMOV_V64I_FLOAT2INT:
		{
			...
			il.AddInstruction(il.SetRegister(get_register_size(minreg), minreg, il.FloatToInt(REGSZ_O(operand1), ILREG_O(instr.operands[1]))));
			break;
		}
	...
	case ARM64_LDADDAL:
		...
		il.AddInstruction(il.Store(REGSZ_O(operand3), ILREG_O(operand3), il.Add(REGSZ_O(operand1), ILREG_O(operand1), ILREG_O(operand2))));
		break;
	case ARM64_LDADDB:
	case ARM64_LDADDAB:
	case ARM64_LDADDLB:
	case ARM64_LDADDALB:
		...
		il.AddInstruction(il.Store(REGSZ_O(operand3), ILREG_O(operand3), il.Add(1, il.LowPart(1, ILREG_O(operand1)), il.LowPart(1, ILREG_O(operand2)))));
		break;
	case ARM64_LDADDH:
	case ARM64_LDADDAH:
	case ARM64_LDADDLH:
	case ARM64_LDADDALH:
		...
		il.AddInstruction(il.Store(REGSZ_O(operand3), ILREG_O(operand3), il.Add(2, il.LowPart(2, ILREG_O(operand1)), il.LowPart(2, ILREG_O(operand2)))));
		break;
	case ARM64_LSL:
		il.AddInstruction(ILSETREG_O(operand1,
	...
	case ARM64_MOVK:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.Or(REGSZ_O(operand1), ILREG_O(operand1), il.Const(REGSZ_O(operand1), IMM_O(operand2) << operand2.shiftValue))));
		break;
	case ARM64_MOVZ:
		il.AddInstruction(ILSETREG_O(operand1,
	...
	case ARM64_MSUB:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.Sub(REGSZ_O(operand1), ILREG_O(operand4), il.Mult(REGSZ_O(operand1), ILREG_O(operand2), ILREG_O(operand3)))));
		break;
	case ARM64_MNEG:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.Sub(REGSZ_O(operand1),
		                                    il.Const(8, 0),
		                                    il.Mult(REGSZ_O(operand1), ILREG_O(operand2), ILREG_O(operand3)))));
		break;
	case ARM64_MSR:
	...
	case ARM64_NEGS:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.Neg(REGSZ_O(operand1), ReadILOperand(il, instr.operands[1], REGSZ_O(operand1)), SETFLAGS)));
		break;
	case ARM64_NOP:
	...
	case ARM64_REV:
		...
		il.AddInstruction(il.Intrinsic({RegisterOrFlag::Register(REG_O(operand1))}, ARM64_INTRIN_REV, {ILREG_O(operand2)}));
		break;
	case ARM64_RBIT:
		il.AddInstruction(il.Intrinsic({RegisterOrFlag::Register(REG_O(operand1))}, ARM64_INTRIN_RBIT, {ILREG_O(operand2)}));
		break;
	case ARM64_ROR:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.RotateRight(REGSZ_O(operand2),
		                                            ILREG_O(operand2),
		                                            ReadILOperand(il, operand3, REGSZ_O(operand2)))));
		break;
	case ARM64_SBC:
	...
	case ARM64_SBFIZ:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.ArithShiftRight(REGSZ_O(operand1),
		                                                il.ShiftLeft(REGSZ_O(operand1),
		                                                             ExtractBits(il, operand2, IMM_O(operand4), 0),
	...
	case ARM64_SDIV:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.DivSigned(REGSZ_O(operand2), ILREG_O(operand2), ILREG_O(operand3))));
		break;
	case ARM64_SEV:
	...
	case ARM64_SHL:
	{
		...
		for (int i = 0; i < dst_n; ++i)
		{
			il.AddInstruction(il.SetRegister(rsize, dsts[i], il.ShiftLeft(rsize, il.Register(rsize, srcs[i]), il.Const(0, IMM_O(operand3)))));
		}
	...
	case ARM64_SUBS:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.Sub(REGSZ_O(operand1), ILREG_O(operand2), ReadILOperand(il, instr.operands[2], REGSZ_O(operand1)), SETFLAGS)));
		break;
	case ARM64_SVC:
	case ARM64_HVC:
	case ARM64_SMC:
	...
	case ARM64_SXTB:
		il.AddInstruction(ILSETREG_O(operand1,
		                             ExtractRegister(il, operand2, 0, 1, true, REGSZ_O(operand1))));
		break;
	case ARM64_SXTH:
		il.AddInstruction(ILSETREG_O(operand1,
		                             ExtractRegister(il, operand2, 0, 2, true, REGSZ_O(operand1))));
		break;
	case ARM64_SXTW:
		il.AddInstruction(ILSETREG_O(operand1,
		                             ExtractRegister(il, operand2, 0, 4, true, REGSZ_O(operand1))));
		break;
	case ARM64_TBNZ:
		ConditionalJump(arch, il, il.CompareNotEqual(REGSZ_O(operand1), ExtractBit(il, operand1, IMM_O(operand2)), il.Const(REGSZ_O(operand1), 0)), addrSize, IMM_O(operand3), addr + 4);
		return false;
	case ARM64_TBZ:
		ConditionalJump(arch, il, il.CompareEqual(REGSZ_O(operand1), ExtractBit(il, operand1, IMM_O(operand2)), il.Const(REGSZ_O(operand1), 0)), addrSize, IMM_O(operand3), addr + 4);
		return false;
	case ARM64_TST:
		il.AddInstruction(il.And(REGSZ_O(operand1),
		                         ILREG_O(operand1),
		                         ReadILOperand(il, operand2, REGSZ_O(operand1)),
		                         SETFLAGS));
		break;
	...
	case ARM64_USHR:
	{
		...
		for (int i = 0; i < dst_n; ++i)
		{
			il.AddInstruction(il.SetRegister(rsize, dsts[i], il.LogicalShiftRight(rsize, il.Register(rsize, srcs[i]), il.Const(0, IMM_O(operand3)))));
		}
	...
	case ARM64_UDIV:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.DivUnsigned(REGSZ_O(operand2), ILREG_O(operand2), ILREG_O(operand3))));
		break;
	case ARM64_UBFIZ:
		il.AddInstruction(ILSETREG_O(operand1,
		                             il.ZeroExtend(REGSZ_O(operand1), il.ShiftLeft(REGSZ_O(operand2), il.And(REGSZ_O(operand2), ILREG_O(operand2), il.Const(REGSZ_O(operand2), (1LL << IMM_O(operand4)) - 1)), il.Const(1, IMM_O(operand3))))));
		break;
	case ARM64_UBFX:
	{
		...
		{
			il.AddInstruction(ILSETREG_O(operand1,
			                             il.LowPart(src_len / 8, ILREG_O(operand2))));
		}
		else
		{
			il.AddInstruction(ILSETREG_O(operand1,
			                             il.ZeroExtend(REGSZ_O(operand1), il.And(REGSZ_O(operand2), il.LogicalShiftRight(REGSZ_O(operand2), ILREG_O(operand2), il.Const(1, IMM_O(operand3))), il.Const(REGSZ_O(operand2), (1LL << IMM_O(operand4)) - 1)))));
		}
		break;
	}
	case ARM64_UXTB:
		il.AddInstruction(ILSETREG_O(operand1,
		                             ExtractRegister(il, operand2, 0, 1, false, REGSZ_O(operand1))));
		break;
	case ARM64_UXTH:
		il.AddInstruction(ILSETREG_O(operand1,
		                             ExtractRegister(il, operand2, 0, 2, false, REGSZ_O(operand1))));
		break;
	case ARM64_WFE:
