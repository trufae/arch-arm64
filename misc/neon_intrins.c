...
float32x2_t vmulx_lane_f32(float32x2_t a, float32x2_t v, const int lane);                          // FMULX Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vmulxq_lane_f32(float32x4_t a, float32x2_t v, const int lane);                         // FMULX Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vmulx_lane_f64(float64x1_t a, float64x1_t v, const int lane);                          // FMULX Dd,Dn,Vm.D[lane]
float64x2_t vmulxq_lane_f64(float64x2_t a, float64x1_t v, const int lane);                         // FMULX Vd.2D,Vn.2D,Vm.D[lane]
...
float32x2_t vmulx_laneq_f32(float32x2_t a, float32x4_t v, const int lane);                         // FMULX Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vmulxq_laneq_f32(float32x4_t a, float32x4_t v, const int lane);                        // FMULX Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vmulx_laneq_f64(float64x1_t a, float64x2_t v, const int lane);                         // FMULX Dd,Dn,Vm.D[lane]
float64x2_t vmulxq_laneq_f64(float64x2_t a, float64x2_t v, const int lane);                        // FMULX Vd.2D,Vn.2D,Vm.D[lane]
...
float32x2_t vmla_f32(float32x2_t a, float32x2_t b, float32x2_t c);                                 // RESULT[I] = a[i] + (b[i] * c[i]) for i = 0 to 1
float32x4_t vmlaq_f32(float32x4_t a, float32x4_t b, float32x4_t c);                                // RESULT[I] = a[i] + (b[i] * c[i]) for i = 0 to 3
float64x1_t vmla_f64(float64x1_t a, float64x1_t b, float64x1_t c);                                 // RESULT[I] = a[i] + (b[i] * c[i]) for i = 0
float64x2_t vmlaq_f64(float64x2_t a, float64x2_t b, float64x2_t c);                                // RESULT[I] = a[i] + (b[i] * c[i]) for i = 0 to 1
...
float32x2_t vmls_f32(float32x2_t a, float32x2_t b, float32x2_t c);                                 // RESULT[I] = a[i] - (b[i] * c[i]) for i = 0 to 1
float32x4_t vmlsq_f32(float32x4_t a, float32x4_t b, float32x4_t c);                                // RESULT[I] = a[i] - (b[i] * c[i]) for i = 0 to 3
float64x1_t vmls_f64(float64x1_t a, float64x1_t b, float64x1_t c);                                 // RESULT[I] = a[i] - (b[i] * c[i]) for i = 0
float64x2_t vmlsq_f64(float64x2_t a, float64x2_t b, float64x2_t c);                                // RESULT[I] = a[i] - (b[i] * c[i]) for i = 0 to 1
...
float32x2_t vfma_lane_f32(float32x2_t a, float32x2_t b, float32x2_t v, const int lane);            // FMLA Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vfmaq_lane_f32(float32x4_t a, float32x4_t b, float32x2_t v, const int lane);           // FMLA Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vfma_lane_f64(float64x1_t a, float64x1_t b, float64x1_t v, const int lane);            // FMLA Dd,Dn,Vm.D[lane]
float64x2_t vfmaq_lane_f64(float64x2_t a, float64x2_t b, float64x1_t v, const int lane);           // FMLA Vd.2D,Vn.2D,Vm.D[lane]
float32_t vfmas_lane_f32(float32_t a, float32_t b, float32x2_t v, const int lane);                 // FMLA Sd,Sn,Vm.S[lane]
float64_t vfmad_lane_f64(float64_t a, float64_t b, float64x1_t v, const int lane);                 // FMLA Dd,Dn,Vm.D[lane]
float32x2_t vfma_laneq_f32(float32x2_t a, float32x2_t b, float32x4_t v, const int lane);           // FMLA Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vfmaq_laneq_f32(float32x4_t a, float32x4_t b, float32x4_t v, const int lane);          // FMLA Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vfma_laneq_f64(float64x1_t a, float64x1_t b, float64x2_t v, const int lane);           // FMLA Dd,Dn,Vm.D[lane]
float64x2_t vfmaq_laneq_f64(float64x2_t a, float64x2_t b, float64x2_t v, const int lane);          // FMLA Vd.2D,Vn.2D,Vm.D[lane]
float32_t vfmas_laneq_f32(float32_t a, float32_t b, float32x4_t v, const int lane);                // FMLA Sd,Sn,Vm.S[lane]
float64_t vfmad_laneq_f64(float64_t a, float64_t b, float64x2_t v, const int lane);                // FMLA Dd,Dn,Vm.D[lane]
...
float32x2_t vfms_lane_f32(float32x2_t a, float32x2_t b, float32x2_t v, const int lane);            // FMLS Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vfmsq_lane_f32(float32x4_t a, float32x4_t b, float32x2_t v, const int lane);           // FMLS Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vfms_lane_f64(float64x1_t a, float64x1_t b, float64x1_t v, const int lane);            // FMLS Dd,Dn,Vm.D[lane]
float64x2_t vfmsq_lane_f64(float64x2_t a, float64x2_t b, float64x1_t v, const int lane);           // FMLS Vd.2D,Vn.2D,Vm.D[lane]
float32_t vfmss_lane_f32(float32_t a, float32_t b, float32x2_t v, const int lane);                 // FMLS Sd,Sn,Vm.S[lane]
float64_t vfmsd_lane_f64(float64_t a, float64_t b, float64x1_t v, const int lane);                 // FMLS Dd,Dn,Vm.D[lane]
float32x2_t vfms_laneq_f32(float32x2_t a, float32x2_t b, float32x4_t v, const int lane);           // FMLS Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vfmsq_laneq_f32(float32x4_t a, float32x4_t b, float32x4_t v, const int lane);          // FMLS Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vfms_laneq_f64(float64x1_t a, float64x1_t b, float64x2_t v, const int lane);           // FMLS Dd,Dn,Vm.D[lane]
float64x2_t vfmsq_laneq_f64(float64x2_t a, float64x2_t b, float64x2_t v, const int lane);          // FMLS Vd.2D,Vn.2D,Vm.D[lane]
float32_t vfmss_laneq_f32(float32_t a, float32_t b, float32x4_t v, const int lane);                // FMLS Sd,Sn,Vm.S[lane]
float64_t vfmsd_laneq_f64(float64_t a, float64_t b, float64x2_t v, const int lane);                // FMLS Dd,Dn,Vm.D[lane]
...
int16x4_t vmla_lane_s16(int16x4_t a, int16x4_t b, int16x4_t v, const int lane);                    // MLA Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vmlaq_lane_s16(int16x8_t a, int16x8_t b, int16x4_t v, const int lane);                   // MLA Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vmla_lane_s32(int32x2_t a, int32x2_t b, int32x2_t v, const int lane);                    // MLA Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vmlaq_lane_s32(int32x4_t a, int32x4_t b, int32x2_t v, const int lane);                   // MLA Vd.4S,Vn.4S,Vm.S[lane]
uint16x4_t vmla_lane_u16(uint16x4_t a, uint16x4_t b, uint16x4_t v, const int lane);                // MLA Vd.4H,Vn.4H,Vm.H[lane]
uint16x8_t vmlaq_lane_u16(uint16x8_t a, uint16x8_t b, uint16x4_t v, const int lane);               // MLA Vd.8H,Vn.8H,Vm.H[lane]
uint32x2_t vmla_lane_u32(uint32x2_t a, uint32x2_t b, uint32x2_t v, const int lane);                // MLA Vd.2S,Vn.2S,Vm.S[lane]
uint32x4_t vmlaq_lane_u32(uint32x4_t a, uint32x4_t b, uint32x2_t v, const int lane);               // MLA Vd.4S,Vn.4S,Vm.S[lane]
float32x2_t vmla_lane_f32(float32x2_t a, float32x2_t b, float32x2_t v, const int lane);            // RESULT[I] = a[i] + (b[i] * v[lane]) for i = 0 to 1
float32x4_t vmlaq_lane_f32(float32x4_t a, float32x4_t b, float32x2_t v, const int lane);           // RESULT[I] = a[i] + (b[i] * v[lane]) for i = 0 to 3
int16x4_t vmla_laneq_s16(int16x4_t a, int16x4_t b, int16x8_t v, const int lane);                   // MLA Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vmlaq_laneq_s16(int16x8_t a, int16x8_t b, int16x8_t v, const int lane);                  // MLA Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vmla_laneq_s32(int32x2_t a, int32x2_t b, int32x4_t v, const int lane);                   // MLA Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vmlaq_laneq_s32(int32x4_t a, int32x4_t b, int32x4_t v, const int lane);                  // MLA Vd.4S,Vn.4S,Vm.S[lane]
uint16x4_t vmla_laneq_u16(uint16x4_t a, uint16x4_t b, uint16x8_t v, const int lane);               // MLA Vd.4H,Vn.4H,Vm.H[lane]
uint16x8_t vmlaq_laneq_u16(uint16x8_t a, uint16x8_t b, uint16x8_t v, const int lane);              // MLA Vd.8H,Vn.8H,Vm.H[lane]
uint32x2_t vmla_laneq_u32(uint32x2_t a, uint32x2_t b, uint32x4_t v, const int lane);               // MLA Vd.2S,Vn.2S,Vm.S[lane]
uint32x4_t vmlaq_laneq_u32(uint32x4_t a, uint32x4_t b, uint32x4_t v, const int lane);              // MLA Vd.4S,Vn.4S,Vm.S[lane]
float32x2_t vmla_laneq_f32(float32x2_t a, float32x2_t b, float32x4_t v, const int lane);           // RESULT[I] = a[i] + (b[i] * v[lane]) for i = 0 to 1
float32x4_t vmlaq_laneq_f32(float32x4_t a, float32x4_t b, float32x4_t v, const int lane);          // RESULT[I] = a[i] + (b[i] * v[lane]) for i = 0 to 3
int32x4_t vmlal_lane_s16(int32x4_t a, int16x4_t b, int16x4_t v, const int lane);                   // SMLAL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vmlal_lane_s32(int64x2_t a, int32x2_t b, int32x2_t v, const int lane);                   // SMLAL Vd.2D,Vn.2S,Vm.S[lane]
uint32x4_t vmlal_lane_u16(uint32x4_t a, uint16x4_t b, uint16x4_t v, const int lane);               // UMLAL Vd.4S,Vn.4H,Vm.H[lane]
uint64x2_t vmlal_lane_u32(uint64x2_t a, uint32x2_t b, uint32x2_t v, const int lane);               // UMLAL Vd.2D,Vn.2S,Vm.S[lane]
int32x4_t vmlal_high_lane_s16(int32x4_t a, int16x8_t b, int16x4_t v, const int lane);              // SMLAL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vmlal_high_lane_s32(int64x2_t a, int32x4_t b, int32x2_t v, const int lane);              // SMLAL2 Vd.2D,Vn.4S,Vm.S[lane]
uint32x4_t vmlal_high_lane_u16(uint32x4_t a, uint16x8_t b, uint16x4_t v, const int lane);          // UMLAL2 Vd.4S,Vn.8H,Vm.H[lane]
uint64x2_t vmlal_high_lane_u32(uint64x2_t a, uint32x4_t b, uint32x2_t v, const int lane);          // UMLAL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vmlal_laneq_s16(int32x4_t a, int16x4_t b, int16x8_t v, const int lane);                  // SMLAL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vmlal_laneq_s32(int64x2_t a, int32x2_t b, int32x4_t v, const int lane);                  // SMLAL Vd.2D,Vn.2S,Vm.S[lane]
uint32x4_t vmlal_laneq_u16(uint32x4_t a, uint16x4_t b, uint16x8_t v, const int lane);              // UMLAL Vd.4S,Vn.4H,Vm.H[lane]
uint64x2_t vmlal_laneq_u32(uint64x2_t a, uint32x2_t b, uint32x4_t v, const int lane);              // UMLAL Vd.2D,Vn.2S,Vm.S[lane]
int32x4_t vmlal_high_laneq_s16(int32x4_t a, int16x8_t b, int16x8_t v, const int lane);             // SMLAL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vmlal_high_laneq_s32(int64x2_t a, int32x4_t b, int32x4_t v, const int lane);             // SMLAL2 Vd.2D,Vn.4S,Vm.S[lane]
uint32x4_t vmlal_high_laneq_u16(uint32x4_t a, uint16x8_t b, uint16x8_t v, const int lane);         // UMLAL2 Vd.4S,Vn.8H,Vm.H[lane]
uint64x2_t vmlal_high_laneq_u32(uint64x2_t a, uint32x4_t b, uint32x4_t v, const int lane);         // UMLAL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vqdmlal_lane_s16(int32x4_t a, int16x4_t b, int16x4_t v, const int lane);                 // SQDMLAL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vqdmlal_lane_s32(int64x2_t a, int32x2_t b, int32x2_t v, const int lane);                 // SQDMLAL Vd.2D,Vn.2S,Vm.S[lane]
int32_t vqdmlalh_lane_s16(int32_t a, int16_t b, int16x4_t v, const int lane);                      // SQDMLAL Sd,Hn,Vm.H[lane]
int64_t vqdmlals_lane_s32(int64_t a, int32_t b, int32x2_t v, const int lane);                      // SQDMLAL Dd,Sn,Vm.S[lane]
int32x4_t vqdmlal_high_lane_s16(int32x4_t a, int16x8_t b, int16x4_t v, const int lane);            // SQDMLAL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vqdmlal_high_lane_s32(int64x2_t a, int32x4_t b, int32x2_t v, const int lane);            // SQDMLAL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vqdmlal_laneq_s16(int32x4_t a, int16x4_t b, int16x8_t v, const int lane);                // SQDMLAL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vqdmlal_laneq_s32(int64x2_t a, int32x2_t b, int32x4_t v, const int lane);                // SQDMLAL Vd.2D,Vn.2S,Vm.S[lane]
int32_t vqdmlalh_laneq_s16(int32_t a, int16_t b, int16x8_t v, const int lane);                     // SQDMLAL Sd,Hn,Vm.H[lane]
int64_t vqdmlals_laneq_s32(int64_t a, int32_t b, int32x4_t v, const int lane);                     // SQDMLAL Dd,Sn,Vm.S[lane]
int32x4_t vqdmlal_high_laneq_s16(int32x4_t a, int16x8_t b, int16x8_t v, const int lane);           // SQDMLAL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vqdmlal_high_laneq_s32(int64x2_t a, int32x4_t b, int32x4_t v, const int lane);           // SQDMLAL2 Vd.2D,Vn.4S,Vm.S[lane]
int16x4_t vmls_lane_s16(int16x4_t a, int16x4_t b, int16x4_t v, const int lane);                    // MLS Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vmlsq_lane_s16(int16x8_t a, int16x8_t b, int16x4_t v, const int lane);                   // MLS Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vmls_lane_s32(int32x2_t a, int32x2_t b, int32x2_t v, const int lane);                    // MLS Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vmlsq_lane_s32(int32x4_t a, int32x4_t b, int32x2_t v, const int lane);                   // MLS Vd.4S,Vn.4S,Vm.S[lane]
uint16x4_t vmls_lane_u16(uint16x4_t a, uint16x4_t b, uint16x4_t v, const int lane);                // MLS Vd.4H,Vn.4H,Vm.H[lane]
uint16x8_t vmlsq_lane_u16(uint16x8_t a, uint16x8_t b, uint16x4_t v, const int lane);               // MLS Vd.8H,Vn.8H,Vm.H[lane]
uint32x2_t vmls_lane_u32(uint32x2_t a, uint32x2_t b, uint32x2_t v, const int lane);                // MLS Vd.2S,Vn.2S,Vm.S[lane]
uint32x4_t vmlsq_lane_u32(uint32x4_t a, uint32x4_t b, uint32x2_t v, const int lane);               // MLS Vd.4S,Vn.4S,Vm.S[lane]
float32x2_t vmls_lane_f32(float32x2_t a, float32x2_t b, float32x2_t v, const int lane);            // RESULT[I] = a[i] - (b[i] * v[lane]) for i = 0 to 1
float32x4_t vmlsq_lane_f32(float32x4_t a, float32x4_t b, float32x2_t v, const int lane);           // RESULT[I] = a[i] - (b[i] * v[lane]) for i = 0 to 3
int16x4_t vmls_laneq_s16(int16x4_t a, int16x4_t b, int16x8_t v, const int lane);                   // MLS Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vmlsq_laneq_s16(int16x8_t a, int16x8_t b, int16x8_t v, const int lane);                  // MLS Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vmls_laneq_s32(int32x2_t a, int32x2_t b, int32x4_t v, const int lane);                   // MLS Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vmlsq_laneq_s32(int32x4_t a, int32x4_t b, int32x4_t v, const int lane);                  // MLS Vd.4S,Vn.4S,Vm.S[lane]
uint16x4_t vmls_laneq_u16(uint16x4_t a, uint16x4_t b, uint16x8_t v, const int lane);               // MLS Vd.4H,Vn.4H,Vm.H[lane]
uint16x8_t vmlsq_laneq_u16(uint16x8_t a, uint16x8_t b, uint16x8_t v, const int lane);              // MLS Vd.8H,Vn.8H,Vm.H[lane]
uint32x2_t vmls_laneq_u32(uint32x2_t a, uint32x2_t b, uint32x4_t v, const int lane);               // MLS Vd.2S,Vn.2S,Vm.S[lane]
uint32x4_t vmlsq_laneq_u32(uint32x4_t a, uint32x4_t b, uint32x4_t v, const int lane);              // MLS Vd.4S,Vn.4S,Vm.S[lane]
float32x2_t vmls_laneq_f32(float32x2_t a, float32x2_t b, float32x4_t v, const int lane);           // RESULT[I] = a[i] - (b[i] * v[lane]) for i = 0 to 1
float32x4_t vmlsq_laneq_f32(float32x4_t a, float32x4_t b, float32x4_t v, const int lane);          // RESULT[I] = a[i] - (b[i] * v[lane]) for i = 0 to 3
int32x4_t vmlsl_lane_s16(int32x4_t a, int16x4_t b, int16x4_t v, const int lane);                   // SMLSL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vmlsl_lane_s32(int64x2_t a, int32x2_t b, int32x2_t v, const int lane);                   // SMLSL Vd.2D,Vn.2S,Vm.S[lane]
uint32x4_t vmlsl_lane_u16(uint32x4_t a, uint16x4_t b, uint16x4_t v, const int lane);               // UMLSL Vd.4S,Vn.4H,Vm.H[lane]
uint64x2_t vmlsl_lane_u32(uint64x2_t a, uint32x2_t b, uint32x2_t v, const int lane);               // UMLSL Vd.2D,Vn.2S,Vm.S[lane]
int32x4_t vmlsl_high_lane_s16(int32x4_t a, int16x8_t b, int16x4_t v, const int lane);              // SMLSL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vmlsl_high_lane_s32(int64x2_t a, int32x4_t b, int32x2_t v, const int lane);              // SMLSL2 Vd.2D,Vn.4S,Vm.S[lane]
uint32x4_t vmlsl_high_lane_u16(uint32x4_t a, uint16x8_t b, uint16x4_t v, const int lane);          // UMLSL2 Vd.4S,Vn.8H,Vm.H[lane]
uint64x2_t vmlsl_high_lane_u32(uint64x2_t a, uint32x4_t b, uint32x2_t v, const int lane);          // UMLSL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vmlsl_laneq_s16(int32x4_t a, int16x4_t b, int16x8_t v, const int lane);                  // SMLSL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vmlsl_laneq_s32(int64x2_t a, int32x2_t b, int32x4_t v, const int lane);                  // SMLSL Vd.2D,Vn.2S,Vm.S[lane]
uint32x4_t vmlsl_laneq_u16(uint32x4_t a, uint16x4_t b, uint16x8_t v, const int lane);              // UMLSL Vd.4S,Vn.4H,Vm.H[lane]
uint64x2_t vmlsl_laneq_u32(uint64x2_t a, uint32x2_t b, uint32x4_t v, const int lane);              // UMLSL Vd.2D,Vn.2S,Vm.S[lane]
int32x4_t vmlsl_high_laneq_s16(int32x4_t a, int16x8_t b, int16x8_t v, const int lane);             // SMLSL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vmlsl_high_laneq_s32(int64x2_t a, int32x4_t b, int32x4_t v, const int lane);             // SMLSL2 Vd.2D,Vn.4S,Vm.S[lane]
uint32x4_t vmlsl_high_laneq_u16(uint32x4_t a, uint16x8_t b, uint16x8_t v, const int lane);         // UMLSL2 Vd.4S,Vn.8H,Vm.H[lane]
uint64x2_t vmlsl_high_laneq_u32(uint64x2_t a, uint32x4_t b, uint32x4_t v, const int lane);         // UMLSL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vqdmlsl_lane_s16(int32x4_t a, int16x4_t b, int16x4_t v, const int lane);                 // SQDMLSL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vqdmlsl_lane_s32(int64x2_t a, int32x2_t b, int32x2_t v, const int lane);                 // SQDMLSL Vd.2D,Vn.2S,Vm.S[lane]
int32_t vqdmlslh_lane_s16(int32_t a, int16_t b, int16x4_t v, const int lane);                      // SQDMLSL Sd,Hn,Vm.H[lane]
int64_t vqdmlsls_lane_s32(int64_t a, int32_t b, int32x2_t v, const int lane);                      // SQDMLSL Dd,Sn,Vm.S[lane]
int32x4_t vqdmlsl_high_lane_s16(int32x4_t a, int16x8_t b, int16x4_t v, const int lane);            // SQDMLSL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vqdmlsl_high_lane_s32(int64x2_t a, int32x4_t b, int32x2_t v, const int lane);            // SQDMLSL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vqdmlsl_laneq_s16(int32x4_t a, int16x4_t b, int16x8_t v, const int lane);                // SQDMLSL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vqdmlsl_laneq_s32(int64x2_t a, int32x2_t b, int32x4_t v, const int lane);                // SQDMLSL Vd.2D,Vn.2S,Vm.S[lane]
int32_t vqdmlslh_laneq_s16(int32_t a, int16_t b, int16x8_t v, const int lane);                     // SQDMLSL Sd,Hn,Vm.H[lane]
int64_t vqdmlsls_laneq_s32(int64_t a, int32_t b, int32x4_t v, const int lane);                     // SQDMLSL Dd,Sn,Vm.S[lane]
int32x4_t vqdmlsl_high_laneq_s16(int32x4_t a, int16x8_t b, int16x8_t v, const int lane);           // SQDMLSL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vqdmlsl_high_laneq_s32(int64x2_t a, int32x4_t b, int32x4_t v, const int lane);           // SQDMLSL2 Vd.2D,Vn.4S,Vm.S[lane]
...
uint16x8_t vmulq_lane_u16(uint16x8_t a, uint16x4_t v, const int lane);                             // MUL Vd.8H,Vn.8H,Vm.H[lane]
uint32x2_t vmul_lane_u32(uint32x2_t a, uint32x2_t v, const int lane);                              // MUL Vd.2S,Vn.2S,Vm.S[lane]
uint32x4_t vmulq_lane_u32(uint32x4_t a, uint32x2_t v, const int lane);                             // MUL Vd.4S,Vn.4S,Vm.S[lane]
float32x2_t vmul_lane_f32(float32x2_t a, float32x2_t v, const int lane);                           // FMUL Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vmulq_lane_f32(float32x4_t a, float32x2_t v, const int lane);                          // FMUL Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vmul_lane_f64(float64x1_t a, float64x1_t v, const int lane);                           // FMUL Dd,Dn,Vm.D[lane]
float64x2_t vmulq_lane_f64(float64x2_t a, float64x1_t v, const int lane);                          // FMUL Vd.2D,Vn.2D,Vm.D[lane]
...
uint16x4_t vmul_laneq_u16(uint16x4_t a, uint16x8_t v, const int lane);                             // MUL Vd.4H,Vn.4H,Vm.H[lane]
uint16x8_t vmulq_laneq_u16(uint16x8_t a, uint16x8_t v, const int lane);                            // MUL Vd.8H,Vn.8H,Vm.H[lane]
uint32x2_t vmul_laneq_u32(uint32x2_t a, uint32x4_t v, const int lane);                             // MUL Vd.2S,Vn.2S,Vm.S[lane]
uint32x4_t vmulq_laneq_u32(uint32x4_t a, uint32x4_t v, const int lane);                            // MUL Vd.4S,Vn.4S,Vm.S[lane]
float32x2_t vmul_laneq_f32(float32x2_t a, float32x4_t v, const int lane);                          // FMUL Vd.2S,Vn.2S,Vm.S[lane]
float32x4_t vmulq_laneq_f32(float32x4_t a, float32x4_t v, const int lane);                         // FMUL Vd.4S,Vn.4S,Vm.S[lane]
float64x1_t vmul_laneq_f64(float64x1_t a, float64x2_t v, const int lane);                          // FMUL Dd,Dn,Vm.D[lane]
float64x2_t vmulq_laneq_f64(float64x2_t a, float64x2_t v, const int lane);                         // FMUL Vd.2D,Vn.2D,Vm.D[lane]
...
uint32x4_t vmull_lane_u16(uint16x4_t a, uint16x4_t v, const int lane);                             // UMULL Vd.4S,Vn.4H,Vm.H[lane]
uint64x2_t vmull_lane_u32(uint32x2_t a, uint32x2_t v, const int lane);                             // UMULL Vd.2D,Vn.2S,Vm.S[lane]
int32x4_t vmull_high_lane_s16(int16x8_t a, int16x4_t v, const int lane);                           // SMULL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vmull_high_lane_s32(int32x4_t a, int32x2_t v, const int lane);                           // SMULL2 Vd.2D,Vn.4S,Vm.S[lane]
uint32x4_t vmull_high_lane_u16(uint16x8_t a, uint16x4_t v, const int lane);                        // UMULL2 Vd.4S,Vn.8H,Vm.H[lane]
uint64x2_t vmull_high_lane_u32(uint32x4_t a, uint32x2_t v, const int lane);                        // UMULL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vmull_laneq_s16(int16x4_t a, int16x8_t v, const int lane);                               // SMULL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vmull_laneq_s32(int32x2_t a, int32x4_t v, const int lane);                               // SMULL Vd.2D,Vn.2S,Vm.S[lane]
uint32x4_t vmull_laneq_u16(uint16x4_t a, uint16x8_t v, const int lane);                            // UMULL Vd.4S,Vn.4H,Vm.H[lane]
uint64x2_t vmull_laneq_u32(uint32x2_t a, uint32x4_t v, const int lane);                            // UMULL Vd.2D,Vn.2S,Vm.S[lane]
int32x4_t vmull_high_laneq_s16(int16x8_t a, int16x8_t v, const int lane);                          // SMULL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vmull_high_laneq_s32(int32x4_t a, int32x4_t v, const int lane);                          // SMULL2 Vd.2D,Vn.4S,Vm.S[lane]
uint32x4_t vmull_high_laneq_u16(uint16x8_t a, uint16x8_t v, const int lane);                       // UMULL2 Vd.4S,Vn.8H,Vm.H[lane]
uint64x2_t vmull_high_laneq_u32(uint32x4_t a, uint32x4_t v, const int lane);                       // UMULL2 Vd.2D,Vn.4S,Vm.S[lane]
...
int32x4_t vqdmull_lane_s16(int16x4_t a, int16x4_t v, const int lane);                              // SQDMULL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vqdmull_lane_s32(int32x2_t a, int32x2_t v, const int lane);                              // SQDMULL Vd.2D,Vn.2S,Vm.S[lane]
...
int32x4_t vqdmull_high_lane_s16(int16x8_t a, int16x4_t v, const int lane);                         // SQDMULL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vqdmull_high_lane_s32(int32x4_t a, int32x2_t v, const int lane);                         // SQDMULL2 Vd.2D,Vn.4S,Vm.S[lane]
int32x4_t vqdmull_laneq_s16(int16x4_t a, int16x8_t v, const int lane);                             // SQDMULL Vd.4S,Vn.4H,Vm.H[lane]
int64x2_t vqdmull_laneq_s32(int32x2_t a, int32x4_t v, const int lane);                             // SQDMULL Vd.2D,Vn.2S,Vm.S[lane]
...
int32x4_t vqdmull_high_laneq_s16(int16x8_t a, int16x8_t v, const int lane);                        // SQDMULL2 Vd.4S,Vn.8H,Vm.H[lane]
int64x2_t vqdmull_high_laneq_s32(int32x4_t a, int32x4_t v, const int lane);                        // SQDMULL2 Vd.2D,Vn.4S,Vm.S[lane]
...
int16x4_t vqdmulh_lane_s16(int16x4_t a, int16x4_t v, const int lane);                              // SQDMULH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqdmulhq_lane_s16(int16x8_t a, int16x4_t v, const int lane);                             // SQDMULH Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vqdmulh_lane_s32(int32x2_t a, int32x2_t v, const int lane);                              // SQDMULH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqdmulhq_lane_s32(int32x4_t a, int32x2_t v, const int lane);                             // SQDMULH Vd.4S,Vn.4S,Vm.S[lane]
...
int16x4_t vqdmulh_laneq_s16(int16x4_t a, int16x8_t v, const int lane);                             // SQDMULH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqdmulhq_laneq_s16(int16x8_t a, int16x8_t v, const int lane);                            // SQDMULH Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vqdmulh_laneq_s32(int32x2_t a, int32x4_t v, const int lane);                             // SQDMULH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqdmulhq_laneq_s32(int32x4_t a, int32x4_t v, const int lane);                            // SQDMULH Vd.4S,Vn.4S,Vm.S[lane]
...
int16x4_t vqrdmulh_lane_s16(int16x4_t a, int16x4_t v, const int lane);                             // SQRDMULH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqrdmulhq_lane_s16(int16x8_t a, int16x4_t v, const int lane);                            // SQRDMULH Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vqrdmulh_lane_s32(int32x2_t a, int32x2_t v, const int lane);                             // SQRDMULH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqrdmulhq_lane_s32(int32x4_t a, int32x2_t v, const int lane);                            // SQRDMULH Vd.4S,Vn.4S,Vm.S[lane]
...
int16x4_t vqrdmulh_laneq_s16(int16x4_t a, int16x8_t v, const int lane);                            // SQRDMULH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqrdmulhq_laneq_s16(int16x8_t a, int16x8_t v, const int lane);                           // SQRDMULH Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vqrdmulh_laneq_s32(int32x2_t a, int32x4_t v, const int lane);                            // SQRDMULH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqrdmulhq_laneq_s32(int32x4_t a, int32x4_t v, const int lane);                           // SQRDMULH Vd.4S,Vn.4S,Vm.S[lane]
...
float32x2_t vmla_n_f32(float32x2_t a, float32x2_t b, float32_t c);                                 // RESULT[I] = a[i] + (b[i] * c) for i = 0 to 1
float32x4_t vmlaq_n_f32(float32x4_t a, float32x4_t b, float32_t c);                                // RESULT[I] = a[i] + (b[i] * c) for i = 0 to 3
...
float32x2_t vmls_n_f32(float32x2_t a, float32x2_t b, float32_t c);                                 // RESULT[I] = a[i] - (b[i] * c) for i = 0 to 1
float32x4_t vmlsq_n_f32(float32x4_t a, float32x4_t b, float32_t c);                                // RESULT[I] = a[i] - (b[i] * c) for i = 0 to 3
...
int8x8_t vcopy_lane_s8(int8x8_t a, const int lane1, int8x8_t b, const int lane2);                  // INS Vd.B[lane1],Vn.B[lane2]
int8x16_t vcopyq_lane_s8(int8x16_t a, const int lane1, int8x8_t b, const int lane2);               // INS Vd.B[lane1],Vn.B[lane2]
int16x4_t vcopy_lane_s16(int16x4_t a, const int lane1, int16x4_t b, const int lane2);              // INS Vd.H[lane1],Vn.H[lane2]
int16x8_t vcopyq_lane_s16(int16x8_t a, const int lane1, int16x4_t b, const int lane2);             // INS Vd.H[lane1],Vn.H[lane2]
int32x2_t vcopy_lane_s32(int32x2_t a, const int lane1, int32x2_t b, const int lane2);              // INS Vd.S[lane1],Vn.S[lane2]
int32x4_t vcopyq_lane_s32(int32x4_t a, const int lane1, int32x2_t b, const int lane2);             // INS Vd.S[lane1],Vn.S[lane2]
int64x1_t vcopy_lane_s64(int64x1_t a, const int lane1, int64x1_t b, const int lane2);              // DUP Dd,Vn.D[lane2]
int64x2_t vcopyq_lane_s64(int64x2_t a, const int lane1, int64x1_t b, const int lane2);             // INS Vd.D[lane1],Vn.D[lane2]
uint8x8_t vcopy_lane_u8(uint8x8_t a, const int lane1, uint8x8_t b, const int lane2);               // INS Vd.B[lane1],Vn.B[lane2]
uint8x16_t vcopyq_lane_u8(uint8x16_t a, const int lane1, uint8x8_t b, const int lane2);            // INS Vd.B[lane1],Vn.B[lane2]
uint16x4_t vcopy_lane_u16(uint16x4_t a, const int lane1, uint16x4_t b, const int lane2);           // INS Vd.H[lane1],Vn.H[lane2]
uint16x8_t vcopyq_lane_u16(uint16x8_t a, const int lane1, uint16x4_t b, const int lane2);          // INS Vd.H[lane1],Vn.H[lane2]
uint32x2_t vcopy_lane_u32(uint32x2_t a, const int lane1, uint32x2_t b, const int lane2);           // INS Vd.S[lane1],Vn.S[lane2]
uint32x4_t vcopyq_lane_u32(uint32x4_t a, const int lane1, uint32x2_t b, const int lane2);          // INS Vd.S[lane1],Vn.S[lane2]
uint64x1_t vcopy_lane_u64(uint64x1_t a, const int lane1, uint64x1_t b, const int lane2);           // DUP Dd,Vn.D[lane2]
uint64x2_t vcopyq_lane_u64(uint64x2_t a, const int lane1, uint64x1_t b, const int lane2);          // INS Vd.D[lane1],Vn.D[lane2]
poly64x1_t vcopy_lane_p64(poly64x1_t a, const int lane1, poly64x1_t b, const int lane2);           // DUP Dd,Vn.D[lane2]
poly64x2_t vcopyq_lane_p64(poly64x2_t a, const int lane1, poly64x1_t b, const int lane2);          // INS Vd.D[lane1],Vn.D[lane2]
float32x2_t vcopy_lane_f32(float32x2_t a, const int lane1, float32x2_t b, const int lane2);        // INS Vd.S[lane1],Vn.S[lane2]
float32x4_t vcopyq_lane_f32(float32x4_t a, const int lane1, float32x2_t b, const int lane2);       // INS Vd.S[lane1],Vn.S[lane2]
float64x1_t vcopy_lane_f64(float64x1_t a, const int lane1, float64x1_t b, const int lane2);        // DUP Dd,Vn.D[lane2]
float64x2_t vcopyq_lane_f64(float64x2_t a, const int lane1, float64x1_t b, const int lane2);       // INS Vd.D[lane1],Vn.D[lane2]
poly8x8_t vcopy_lane_p8(poly8x8_t a, const int lane1, poly8x8_t b, const int lane2);               // INS Vd.B[lane1],Vn.B[lane2]
poly8x16_t vcopyq_lane_p8(poly8x16_t a, const int lane1, poly8x8_t b, const int lane2);            // INS Vd.B[lane1],Vn.B[lane2]
poly16x4_t vcopy_lane_p16(poly16x4_t a, const int lane1, poly16x4_t b, const int lane2);           // INS Vd.H[lane1],Vn.H[lane2]
poly16x8_t vcopyq_lane_p16(poly16x8_t a, const int lane1, poly16x4_t b, const int lane2);          // INS Vd.H[lane1],Vn.H[lane2]
int8x8_t vcopy_laneq_s8(int8x8_t a, const int lane1, int8x16_t b, const int lane2);                // INS Vd.B[lane1],Vn.B[lane2]
int8x16_t vcopyq_laneq_s8(int8x16_t a, const int lane1, int8x16_t b, const int lane2);             // INS Vd.B[lane1],Vn.B[lane2]
int16x4_t vcopy_laneq_s16(int16x4_t a, const int lane1, int16x8_t b, const int lane2);             // INS Vd.H[lane1],Vn.H[lane2]
int16x8_t vcopyq_laneq_s16(int16x8_t a, const int lane1, int16x8_t b, const int lane2);            // INS Vd.H[lane1],Vn.H[lane2]
int32x2_t vcopy_laneq_s32(int32x2_t a, const int lane1, int32x4_t b, const int lane2);             // INS Vd.S[lane1],Vn.S[lane2]
int32x4_t vcopyq_laneq_s32(int32x4_t a, const int lane1, int32x4_t b, const int lane2);            // INS Vd.S[lane1],Vn.S[lane2]
int64x1_t vcopy_laneq_s64(int64x1_t a, const int lane1, int64x2_t b, const int lane2);             // DUP Dd,Vn.D[lane2]
int64x2_t vcopyq_laneq_s64(int64x2_t a, const int lane1, int64x2_t b, const int lane2);            // INS Vd.D[lane1],Vn.D[lane2]
uint8x8_t vcopy_laneq_u8(uint8x8_t a, const int lane1, uint8x16_t b, const int lane2);             // INS Vd.B[lane1],Vn.B[lane2]
uint8x16_t vcopyq_laneq_u8(uint8x16_t a, const int lane1, uint8x16_t b, const int lane2);          // INS Vd.B[lane1],Vn.B[lane2]
uint16x4_t vcopy_laneq_u16(uint16x4_t a, const int lane1, uint16x8_t b, const int lane2);          // INS Vd.H[lane1],Vn.H[lane2]
uint16x8_t vcopyq_laneq_u16(uint16x8_t a, const int lane1, uint16x8_t b, const int lane2);         // INS Vd.H[lane1],Vn.H[lane2]
uint32x2_t vcopy_laneq_u32(uint32x2_t a, const int lane1, uint32x4_t b, const int lane2);          // INS Vd.S[lane1],Vn.S[lane2]
uint32x4_t vcopyq_laneq_u32(uint32x4_t a, const int lane1, uint32x4_t b, const int lane2);         // INS Vd.S[lane1],Vn.S[lane2]
uint64x1_t vcopy_laneq_u64(uint64x1_t a, const int lane1, uint64x2_t b, const int lane2);          // DUP Dd,Vn.D[lane2]
uint64x2_t vcopyq_laneq_u64(uint64x2_t a, const int lane1, uint64x2_t b, const int lane2);         // INS Vd.D[lane1],Vn.D[lane2]
poly64x1_t vcopy_laneq_p64(poly64x1_t a, const int lane1, poly64x2_t b, const int lane2);          // DUP Dd,Vn.D[lane2]
poly64x2_t vcopyq_laneq_p64(poly64x2_t a, const int lane1, poly64x2_t b, const int lane2);         // INS Vd.D[lane1],Vn.D[lane2]
float32x2_t vcopy_laneq_f32(float32x2_t a, const int lane1, float32x4_t b, const int lane2);       // INS Vd.S[lane1],Vn.S[lane2]
float32x4_t vcopyq_laneq_f32(float32x4_t a, const int lane1, float32x4_t b, const int lane2);      // INS Vd.S[lane1],Vn.S[lane2]
float64x1_t vcopy_laneq_f64(float64x1_t a, const int lane1, float64x2_t b, const int lane2);       // DUP Dd,Vn.D[lane2]
float64x2_t vcopyq_laneq_f64(float64x2_t a, const int lane1, float64x2_t b, const int lane2);      // INS Vd.D[lane1],Vn.D[lane2]
poly8x8_t vcopy_laneq_p8(poly8x8_t a, const int lane1, poly8x16_t b, const int lane2);             // INS Vd.B[lane1],Vn.B[lane2]
poly8x16_t vcopyq_laneq_p8(poly8x16_t a, const int lane1, poly8x16_t b, const int lane2);          // INS Vd.B[lane1],Vn.B[lane2]
poly16x4_t vcopy_laneq_p16(poly16x4_t a, const int lane1, poly16x8_t b, const int lane2);          // INS Vd.H[lane1],Vn.H[lane2]
poly16x8_t vcopyq_laneq_p16(poly16x8_t a, const int lane1, poly16x8_t b, const int lane2);         // INS Vd.H[lane1],Vn.H[lane2]
...
int16x4_t vld1_lane_s16(int16_t const* ptr, int16x4_t src, const int lane);                        // LD1 {Vt.H}[lane],[Xn]
int16x8_t vld1q_lane_s16(int16_t const* ptr, int16x8_t src, const int lane);                       // LD1 {Vt.H}[lane],[Xn]
int32x2_t vld1_lane_s32(int32_t const* ptr, int32x2_t src, const int lane);                        // LD1 {Vt.S}[lane],[Xn]
int32x4_t vld1q_lane_s32(int32_t const* ptr, int32x4_t src, const int lane);                       // LD1 {Vt.S}[lane],[Xn]
int64x1_t vld1_lane_s64(int64_t const* ptr, int64x1_t src, const int lane);                        // LD1 {Vt.D}[lane],[Xn]
int64x2_t vld1q_lane_s64(int64_t const* ptr, int64x2_t src, const int lane);                       // LD1 {Vt.D}[lane],[Xn]
uint8x8_t vld1_lane_u8(uint8_t const* ptr, uint8x8_t src, const int lane);                         // LD1 {Vt.B}[lane],[Xn]
uint8x16_t vld1q_lane_u8(uint8_t const* ptr, uint8x16_t src, const int lane);                      // LD1 {Vt.B}[lane],[Xn]
uint16x4_t vld1_lane_u16(uint16_t const* ptr, uint16x4_t src, const int lane);                     // LD1 {Vt.H}[lane],[Xn]
uint16x8_t vld1q_lane_u16(uint16_t const* ptr, uint16x8_t src, const int lane);                    // LD1 {Vt.H}[lane],[Xn]
uint32x2_t vld1_lane_u32(uint32_t const* ptr, uint32x2_t src, const int lane);                     // LD1 {Vt.S}[lane],[Xn]
uint32x4_t vld1q_lane_u32(uint32_t const* ptr, uint32x4_t src, const int lane);                    // LD1 {Vt.S}[lane],[Xn]
uint64x1_t vld1_lane_u64(uint64_t const* ptr, uint64x1_t src, const int lane);                     // LD1 {Vt.D}[lane],[Xn]
uint64x2_t vld1q_lane_u64(uint64_t const* ptr, uint64x2_t src, const int lane);                    // LD1 {Vt.D}[lane],[Xn]
poly64x1_t vld1_lane_p64(poly64_t const* ptr, poly64x1_t src, const int lane);                     // LD1 {Vt.D}[lane],[Xn]
poly64x2_t vld1q_lane_p64(poly64_t const* ptr, poly64x2_t src, const int lane);                    // LD1 {Vt.D}[lane],[Xn]
float16x4_t vld1_lane_f16(float16_t const* ptr, float16x4_t src, const int lane);                  // LD1 {Vt.H}[lane],[Xn]
float16x8_t vld1q_lane_f16(float16_t const* ptr, float16x8_t src, const int lane);                 // LD1 {Vt.H}[lane],[Xn]
float32x2_t vld1_lane_f32(float32_t const* ptr, float32x2_t src, const int lane);                  // LD1 {Vt.S}[lane],[Xn]
float32x4_t vld1q_lane_f32(float32_t const* ptr, float32x4_t src, const int lane);                 // LD1 {Vt.S}[lane],[Xn]
poly8x8_t vld1_lane_p8(poly8_t const* ptr, poly8x8_t src, const int lane);                         // LD1 {Vt.B}[lane],[Xn]
poly8x16_t vld1q_lane_p8(poly8_t const* ptr, poly8x16_t src, const int lane);                      // LD1 {Vt.B}[lane],[Xn]
poly16x4_t vld1_lane_p16(poly16_t const* ptr, poly16x4_t src, const int lane);                     // LD1 {Vt.H}[lane],[Xn]
poly16x8_t vld1q_lane_p16(poly16_t const* ptr, poly16x8_t src, const int lane);                    // LD1 {Vt.H}[lane],[Xn]
float64x1_t vld1_lane_f64(float64_t const* ptr, float64x1_t src, const int lane);                  // LD1 {Vt.D}[lane],[Xn]
float64x2_t vld1q_lane_f64(float64_t const* ptr, float64x2_t src, const int lane);                 // LD1 {Vt.D}[lane],[Xn]
...
int16x4x2_t vld2_lane_s16(int16_t const* ptr, int16x4x2_t src, const int lane);                    // LD2 {Vt.h - Vt2.h}[lane],[Xn]
int16x8x2_t vld2q_lane_s16(int16_t const* ptr, int16x8x2_t src, const int lane);                   // LD2 {Vt.h - Vt2.h}[lane],[Xn]
int32x2x2_t vld2_lane_s32(int32_t const* ptr, int32x2x2_t src, const int lane);                    // LD2 {Vt.s - Vt2.s}[lane],[Xn]
int32x4x2_t vld2q_lane_s32(int32_t const* ptr, int32x4x2_t src, const int lane);                   // LD2 {Vt.s - Vt2.s}[lane],[Xn]
uint16x4x2_t vld2_lane_u16(uint16_t const* ptr, uint16x4x2_t src, const int lane);                 // LD2 {Vt.h - Vt2.h}[lane],[Xn]
uint16x8x2_t vld2q_lane_u16(uint16_t const* ptr, uint16x8x2_t src, const int lane);                // LD2 {Vt.h - Vt2.h}[lane],[Xn]
uint32x2x2_t vld2_lane_u32(uint32_t const* ptr, uint32x2x2_t src, const int lane);                 // LD2 {Vt.s - Vt2.s}[lane],[Xn]
uint32x4x2_t vld2q_lane_u32(uint32_t const* ptr, uint32x4x2_t src, const int lane);                // LD2 {Vt.s - Vt2.s}[lane],[Xn]
float16x4x2_t vld2_lane_f16(float16_t const* ptr, float16x4x2_t src, const int lane);              // LD2 {Vt.h - Vt2.h}[lane],[Xn]
float16x8x2_t vld2q_lane_f16(float16_t const* ptr, float16x8x2_t src, const int lane);             // LD2 {Vt.h - Vt2.h}[lane],[Xn]
float32x2x2_t vld2_lane_f32(float32_t const* ptr, float32x2x2_t src, const int lane);              // LD2 {Vt.s - Vt2.s}[lane],[Xn]
float32x4x2_t vld2q_lane_f32(float32_t const* ptr, float32x4x2_t src, const int lane);             // LD2 {Vt.s - Vt2.s}[lane],[Xn]
poly16x4x2_t vld2_lane_p16(poly16_t const* ptr, poly16x4x2_t src, const int lane);                 // LD2 {Vt.h - Vt2.h}[lane],[Xn]
poly16x8x2_t vld2q_lane_p16(poly16_t const* ptr, poly16x8x2_t src, const int lane);                // LD2 {Vt.h - Vt2.h}[lane],[Xn]
int8x8x2_t vld2_lane_s8(int8_t const* ptr, int8x8x2_t src, const int lane);                        // LD2 {Vt.b - Vt2.b}[lane],[Xn]
uint8x8x2_t vld2_lane_u8(uint8_t const* ptr, uint8x8x2_t src, const int lane);                     // LD2 {Vt.b - Vt2.b}[lane],[Xn]
poly8x8x2_t vld2_lane_p8(poly8_t const* ptr, poly8x8x2_t src, const int lane);                     // LD2 {Vt.b - Vt2.b}[lane],[Xn]
int8x16x2_t vld2q_lane_s8(int8_t const* ptr, int8x16x2_t src, const int lane);                     // LD2 {Vt.b - Vt2.b}[lane],[Xn]
uint8x16x2_t vld2q_lane_u8(uint8_t const* ptr, uint8x16x2_t src, const int lane);                  // LD2 {Vt.b - Vt2.b}[lane],[Xn]
poly8x16x2_t vld2q_lane_p8(poly8_t const* ptr, poly8x16x2_t src, const int lane);                  // LD2 {Vt.b - Vt2.b}[lane],[Xn]
int64x1x2_t vld2_lane_s64(int64_t const* ptr, int64x1x2_t src, const int lane);                    // LD2 {Vt.d - Vt2.d}[lane],[Xn]
int64x2x2_t vld2q_lane_s64(int64_t const* ptr, int64x2x2_t src, const int lane);                   // LD2 {Vt.d - Vt2.d}[lane],[Xn]
uint64x1x2_t vld2_lane_u64(uint64_t const* ptr, uint64x1x2_t src, const int lane);                 // LD2 {Vt.d - Vt2.d}[lane],[Xn]
uint64x2x2_t vld2q_lane_u64(uint64_t const* ptr, uint64x2x2_t src, const int lane);                // LD2 {Vt.d - Vt2.d}[lane],[Xn]
poly64x1x2_t vld2_lane_p64(poly64_t const* ptr, poly64x1x2_t src, const int lane);                 // LD2 {Vt.d - Vt2.d}[lane],[Xn]
poly64x2x2_t vld2q_lane_p64(poly64_t const* ptr, poly64x2x2_t src, const int lane);                // LD2 {Vt.d - Vt2.d}[lane],[Xn]
float64x1x2_t vld2_lane_f64(float64_t const* ptr, float64x1x2_t src, const int lane);              // LD2 {Vt.d - Vt2.d}[lane],[Xn]
float64x2x2_t vld2q_lane_f64(float64_t const* ptr, float64x2x2_t src, const int lane);             // LD2 {Vt.d - Vt2.d}[lane],[Xn]
int16x4x3_t vld3_lane_s16(int16_t const* ptr, int16x4x3_t src, const int lane);                    // LD3 {Vt.h - Vt3.h}[lane],[Xn]
int16x8x3_t vld3q_lane_s16(int16_t const* ptr, int16x8x3_t src, const int lane);                   // LD3 {Vt.h - Vt3.h}[lane],[Xn]
int32x2x3_t vld3_lane_s32(int32_t const* ptr, int32x2x3_t src, const int lane);                    // LD3 {Vt.s - Vt3.s}[lane],[Xn]
int32x4x3_t vld3q_lane_s32(int32_t const* ptr, int32x4x3_t src, const int lane);                   // LD3 {Vt.s - Vt3.s}[lane],[Xn]
uint16x4x3_t vld3_lane_u16(uint16_t const* ptr, uint16x4x3_t src, const int lane);                 // LD3 {Vt.h - Vt3.h}[lane],[Xn]
uint16x8x3_t vld3q_lane_u16(uint16_t const* ptr, uint16x8x3_t src, const int lane);                // LD3 {Vt.h - Vt3.h}[lane],[Xn]
uint32x2x3_t vld3_lane_u32(uint32_t const* ptr, uint32x2x3_t src, const int lane);                 // LD3 {Vt.s - Vt3.s}[lane],[Xn]
uint32x4x3_t vld3q_lane_u32(uint32_t const* ptr, uint32x4x3_t src, const int lane);                // LD3 {Vt.s - Vt3.s}[lane],[Xn]
float16x4x3_t vld3_lane_f16(float16_t const* ptr, float16x4x3_t src, const int lane);              // LD3 {Vt.h - Vt3.h}[lane],[Xn]
float16x8x3_t vld3q_lane_f16(float16_t const* ptr, float16x8x3_t src, const int lane);             // LD3 {Vt.h - Vt3.h}[lane],[Xn]
float32x2x3_t vld3_lane_f32(float32_t const* ptr, float32x2x3_t src, const int lane);              // LD3 {Vt.s - Vt3.s}[lane],[Xn]
float32x4x3_t vld3q_lane_f32(float32_t const* ptr, float32x4x3_t src, const int lane);             // LD3 {Vt.s - Vt3.s}[lane],[Xn]
poly16x4x3_t vld3_lane_p16(poly16_t const* ptr, poly16x4x3_t src, const int lane);                 // LD3 {Vt.h - Vt3.h}[lane],[Xn]
poly16x8x3_t vld3q_lane_p16(poly16_t const* ptr, poly16x8x3_t src, const int lane);                // LD3 {Vt.h - Vt3.h}[lane],[Xn]
int8x8x3_t vld3_lane_s8(int8_t const* ptr, int8x8x3_t src, const int lane);                        // LD3 {Vt.b - Vt3.b}[lane],[Xn]
uint8x8x3_t vld3_lane_u8(uint8_t const* ptr, uint8x8x3_t src, const int lane);                     // LD3 {Vt.b - Vt3.b}[lane],[Xn]
poly8x8x3_t vld3_lane_p8(poly8_t const* ptr, poly8x8x3_t src, const int lane);                     // LD3 {Vt.b - Vt3.b}[lane],[Xn]
int8x16x3_t vld3q_lane_s8(int8_t const* ptr, int8x16x3_t src, const int lane);                     // LD3 {Vt.b - Vt3.b}[lane],[Xn]
uint8x16x3_t vld3q_lane_u8(uint8_t const* ptr, uint8x16x3_t src, const int lane);                  // LD3 {Vt.b - Vt3.b}[lane],[Xn]
poly8x16x3_t vld3q_lane_p8(poly8_t const* ptr, poly8x16x3_t src, const int lane);                  // LD3 {Vt.b - Vt3.b}[lane],[Xn]
int64x1x3_t vld3_lane_s64(int64_t const* ptr, int64x1x3_t src, const int lane);                    // LD3 {Vt.d - Vt3.d}[lane],[Xn]
int64x2x3_t vld3q_lane_s64(int64_t const* ptr, int64x2x3_t src, const int lane);                   // LD3 {Vt.d - Vt3.d}[lane],[Xn]
uint64x1x3_t vld3_lane_u64(uint64_t const* ptr, uint64x1x3_t src, const int lane);                 // LD3 {Vt.d - Vt3.d}[lane],[Xn]
uint64x2x3_t vld3q_lane_u64(uint64_t const* ptr, uint64x2x3_t src, const int lane);                // LD3 {Vt.d - Vt3.d}[lane],[Xn]
poly64x1x3_t vld3_lane_p64(poly64_t const* ptr, poly64x1x3_t src, const int lane);                 // LD3 {Vt.d - Vt3.d}[lane],[Xn]
poly64x2x3_t vld3q_lane_p64(poly64_t const* ptr, poly64x2x3_t src, const int lane);                // LD3 {Vt.d - Vt3.d}[lane],[Xn]
float64x1x3_t vld3_lane_f64(float64_t const* ptr, float64x1x3_t src, const int lane);              // LD3 {Vt.d - Vt3.d}[lane],[Xn]
float64x2x3_t vld3q_lane_f64(float64_t const* ptr, float64x2x3_t src, const int lane);             // LD3 {Vt.d - Vt3.d}[lane],[Xn]
int16x4x4_t vld4_lane_s16(int16_t const* ptr, int16x4x4_t src, const int lane);                    // LD4 {Vt.h - Vt4.h}[lane],[Xn]
int16x8x4_t vld4q_lane_s16(int16_t const* ptr, int16x8x4_t src, const int lane);                   // LD4 {Vt.h - Vt4.h}[lane],[Xn]
int32x2x4_t vld4_lane_s32(int32_t const* ptr, int32x2x4_t src, const int lane);                    // LD4 {Vt.s - Vt4.s}[lane],[Xn]
int32x4x4_t vld4q_lane_s32(int32_t const* ptr, int32x4x4_t src, const int lane);                   // LD4 {Vt.s - Vt4.s}[lane],[Xn]
uint16x4x4_t vld4_lane_u16(uint16_t const* ptr, uint16x4x4_t src, const int lane);                 // LD4 {Vt.h - Vt4.h}[lane],[Xn]
uint16x8x4_t vld4q_lane_u16(uint16_t const* ptr, uint16x8x4_t src, const int lane);                // LD4 {Vt.h - Vt4.h}[lane],[Xn]
uint32x2x4_t vld4_lane_u32(uint32_t const* ptr, uint32x2x4_t src, const int lane);                 // LD4 {Vt.s - Vt4.s}[lane],[Xn]
uint32x4x4_t vld4q_lane_u32(uint32_t const* ptr, uint32x4x4_t src, const int lane);                // LD4 {Vt.s - Vt4.s}[lane],[Xn]
float16x4x4_t vld4_lane_f16(float16_t const* ptr, float16x4x4_t src, const int lane);              // LD4 {Vt.h - Vt4.h}[lane],[Xn]
float16x8x4_t vld4q_lane_f16(float16_t const* ptr, float16x8x4_t src, const int lane);             // LD4 {Vt.h - Vt4.h}[lane],[Xn]
float32x2x4_t vld4_lane_f32(float32_t const* ptr, float32x2x4_t src, const int lane);              // LD4 {Vt.s - Vt4.s}[lane],[Xn]
float32x4x4_t vld4q_lane_f32(float32_t const* ptr, float32x4x4_t src, const int lane);             // LD4 {Vt.s - Vt4.s}[lane],[Xn]
poly16x4x4_t vld4_lane_p16(poly16_t const* ptr, poly16x4x4_t src, const int lane);                 // LD4 {Vt.h - Vt4.h}[lane],[Xn]
poly16x8x4_t vld4q_lane_p16(poly16_t const* ptr, poly16x8x4_t src, const int lane);                // LD4 {Vt.h - Vt4.h}[lane],[Xn]
int8x8x4_t vld4_lane_s8(int8_t const* ptr, int8x8x4_t src, const int lane);                        // LD4 {Vt.b - Vt4.b}[lane],[Xn]
uint8x8x4_t vld4_lane_u8(uint8_t const* ptr, uint8x8x4_t src, const int lane);                     // LD4 {Vt.b - Vt4.b}[lane],[Xn]
poly8x8x4_t vld4_lane_p8(poly8_t const* ptr, poly8x8x4_t src, const int lane);                     // LD4 {Vt.b - Vt4.b}[lane],[Xn]
int8x16x4_t vld4q_lane_s8(int8_t const* ptr, int8x16x4_t src, const int lane);                     // LD4 {Vt.b - Vt4.b}[lane],[Xn]
uint8x16x4_t vld4q_lane_u8(uint8_t const* ptr, uint8x16x4_t src, const int lane);                  // LD4 {Vt.b - Vt4.b}[lane],[Xn]
poly8x16x4_t vld4q_lane_p8(poly8_t const* ptr, poly8x16x4_t src, const int lane);                  // LD4 {Vt.b - Vt4.b}[lane],[Xn]
int64x1x4_t vld4_lane_s64(int64_t const* ptr, int64x1x4_t src, const int lane);                    // LD4 {Vt.d - Vt4.d}[lane],[Xn]
int64x2x4_t vld4q_lane_s64(int64_t const* ptr, int64x2x4_t src, const int lane);                   // LD4 {Vt.d - Vt4.d}[lane],[Xn]
uint64x1x4_t vld4_lane_u64(uint64_t const* ptr, uint64x1x4_t src, const int lane);                 // LD4 {Vt.d - Vt4.d}[lane],[Xn]
uint64x2x4_t vld4q_lane_u64(uint64_t const* ptr, uint64x2x4_t src, const int lane);                // LD4 {Vt.d - Vt4.d}[lane],[Xn]
poly64x1x4_t vld4_lane_p64(poly64_t const* ptr, poly64x1x4_t src, const int lane);                 // LD4 {Vt.d - Vt4.d}[lane],[Xn]
poly64x2x4_t vld4q_lane_p64(poly64_t const* ptr, poly64x2x4_t src, const int lane);                // LD4 {Vt.d - Vt4.d}[lane],[Xn]
float64x1x4_t vld4_lane_f64(float64_t const* ptr, float64x1x4_t src, const int lane);              // LD4 {Vt.d - Vt4.d}[lane],[Xn]
float64x2x4_t vld4q_lane_f64(float64_t const* ptr, float64x2x4_t src, const int lane);             // LD4 {Vt.d - Vt4.d}[lane],[Xn]
...
void vst2q_lane_s16(int16_t* ptr, int16x8x2_t val, const int lane);                                // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2_lane_s32(int32_t* ptr, int32x2x2_t val, const int lane);                                 // ST2 {Vt.s - Vt2.s}[lane],[Xn]
void vst2q_lane_s32(int32_t* ptr, int32x4x2_t val, const int lane);                                // ST2 {Vt.s - Vt2.s}[lane],[Xn]
void vst2_lane_u16(uint16_t* ptr, uint16x4x2_t val, const int lane);                               // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2q_lane_u16(uint16_t* ptr, uint16x8x2_t val, const int lane);                              // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2_lane_u32(uint32_t* ptr, uint32x2x2_t val, const int lane);                               // ST2 {Vt.s - Vt2.s}[lane],[Xn]
void vst2q_lane_u32(uint32_t* ptr, uint32x4x2_t val, const int lane);                              // ST2 {Vt.s - Vt2.s}[lane],[Xn]
void vst2_lane_f16(float16_t* ptr, float16x4x2_t val, const int lane);                             // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2q_lane_f16(float16_t* ptr, float16x8x2_t val, const int lane);                            // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2_lane_f32(float32_t* ptr, float32x2x2_t val, const int lane);                             // ST2 {Vt.s - Vt2.s}[lane],[Xn]
void vst2q_lane_f32(float32_t* ptr, float32x4x2_t val, const int lane);                            // ST2 {Vt.s - Vt2.s}[lane],[Xn]
void vst2_lane_p16(poly16_t* ptr, poly16x4x2_t val, const int lane);                               // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2q_lane_p16(poly16_t* ptr, poly16x8x2_t val, const int lane);                              // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2q_lane_s8(int8_t* ptr, int8x16x2_t val, const int lane);                                  // ST2 {Vt.b - Vt2.b}[lane],[Xn]
void vst2q_lane_u8(uint8_t* ptr, uint8x16x2_t val, const int lane);                                // ST2 {Vt.b - Vt2.b}[lane],[Xn]
void vst2q_lane_p8(poly8_t* ptr, poly8x16x2_t val, const int lane);                                // ST2 {Vt.b - Vt2.b}[lane],[Xn]
void vst2_lane_s64(int64_t* ptr, int64x1x2_t val, const int lane);                                 // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst2q_lane_s64(int64_t* ptr, int64x2x2_t val, const int lane);                                // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst2_lane_u64(uint64_t* ptr, uint64x1x2_t val, const int lane);                               // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst2q_lane_u64(uint64_t* ptr, uint64x2x2_t val, const int lane);                              // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst2_lane_p64(poly64_t* ptr, poly64x1x2_t val, const int lane);                               // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst2q_lane_p64(poly64_t* ptr, poly64x2x2_t val, const int lane);                              // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst2_lane_f64(float64_t* ptr, float64x1x2_t val, const int lane);                             // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst2q_lane_f64(float64_t* ptr, float64x2x2_t val, const int lane);                            // ST2 {Vt.d - Vt2.d}[lane],[Xn]
void vst3_lane_s16(int16_t* ptr, int16x4x3_t val, const int lane);                                 // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3q_lane_s16(int16_t* ptr, int16x8x3_t val, const int lane);                                // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3_lane_s32(int32_t* ptr, int32x2x3_t val, const int lane);                                 // ST3 {Vt.s - Vt3.s}[lane],[Xn]
void vst3q_lane_s32(int32_t* ptr, int32x4x3_t val, const int lane);                                // ST3 {Vt.s - Vt3.s}[lane],[Xn]
void vst3_lane_u16(uint16_t* ptr, uint16x4x3_t val, const int lane);                               // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3q_lane_u16(uint16_t* ptr, uint16x8x3_t val, const int lane);                              // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3_lane_u32(uint32_t* ptr, uint32x2x3_t val, const int lane);                               // ST3 {Vt.s - Vt3.s}[lane],[Xn]
void vst3q_lane_u32(uint32_t* ptr, uint32x4x3_t val, const int lane);                              // ST3 {Vt.s - Vt3.s}[lane],[Xn]
void vst3_lane_f16(float16_t* ptr, float16x4x3_t val, const int lane);                             // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3q_lane_f16(float16_t* ptr, float16x8x3_t val, const int lane);                            // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3_lane_f32(float32_t* ptr, float32x2x3_t val, const int lane);                             // ST3 {Vt.s - Vt3.s}[lane],[Xn]
void vst3q_lane_f32(float32_t* ptr, float32x4x3_t val, const int lane);                            // ST3 {Vt.s - Vt3.s}[lane],[Xn]
void vst3_lane_p16(poly16_t* ptr, poly16x4x3_t val, const int lane);                               // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3q_lane_p16(poly16_t* ptr, poly16x8x3_t val, const int lane);                              // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3q_lane_s8(int8_t* ptr, int8x16x3_t val, const int lane);                                  // ST3 {Vt.b - Vt3.b}[lane],[Xn]
void vst3q_lane_u8(uint8_t* ptr, uint8x16x3_t val, const int lane);                                // ST3 {Vt.b - Vt3.b}[lane],[Xn]
void vst3q_lane_p8(poly8_t* ptr, poly8x16x3_t val, const int lane);                                // ST3 {Vt.b - Vt3.b}[lane],[Xn]
void vst3_lane_s64(int64_t* ptr, int64x1x3_t val, const int lane);                                 // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst3q_lane_s64(int64_t* ptr, int64x2x3_t val, const int lane);                                // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst3_lane_u64(uint64_t* ptr, uint64x1x3_t val, const int lane);                               // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst3q_lane_u64(uint64_t* ptr, uint64x2x3_t val, const int lane);                              // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst3_lane_p64(poly64_t* ptr, poly64x1x3_t val, const int lane);                               // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst3q_lane_p64(poly64_t* ptr, poly64x2x3_t val, const int lane);                              // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst3_lane_f64(float64_t* ptr, float64x1x3_t val, const int lane);                             // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst3q_lane_f64(float64_t* ptr, float64x2x3_t val, const int lane);                            // ST3 {Vt.d - Vt3.d}[lane],[Xn]
void vst4_lane_s16(int16_t* ptr, int16x4x4_t val, const int lane);                                 // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4q_lane_s16(int16_t* ptr, int16x8x4_t val, const int lane);                                // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4_lane_s32(int32_t* ptr, int32x2x4_t val, const int lane);                                 // ST4 {Vt.s - Vt4.s}[lane],[Xn]
void vst4q_lane_s32(int32_t* ptr, int32x4x4_t val, const int lane);                                // ST4 {Vt.s - Vt4.s}[lane],[Xn]
void vst4_lane_u16(uint16_t* ptr, uint16x4x4_t val, const int lane);                               // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4q_lane_u16(uint16_t* ptr, uint16x8x4_t val, const int lane);                              // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4_lane_u32(uint32_t* ptr, uint32x2x4_t val, const int lane);                               // ST4 {Vt.s - Vt4.s}[lane],[Xn]
void vst4q_lane_u32(uint32_t* ptr, uint32x4x4_t val, const int lane);                              // ST4 {Vt.s - Vt4.s}[lane],[Xn]
void vst4_lane_f16(float16_t* ptr, float16x4x4_t val, const int lane);                             // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4q_lane_f16(float16_t* ptr, float16x8x4_t val, const int lane);                            // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4_lane_f32(float32_t* ptr, float32x2x4_t val, const int lane);                             // ST4 {Vt.s - Vt4.s}[lane],[Xn]
void vst4q_lane_f32(float32_t* ptr, float32x4x4_t val, const int lane);                            // ST4 {Vt.s - Vt4.s}[lane],[Xn]
void vst4_lane_p16(poly16_t* ptr, poly16x4x4_t val, const int lane);                               // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4q_lane_p16(poly16_t* ptr, poly16x8x4_t val, const int lane);                              // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4q_lane_s8(int8_t* ptr, int8x16x4_t val, const int lane);                                  // ST4 {Vt.b - Vt4.b}[lane],[Xn]
void vst4q_lane_u8(uint8_t* ptr, uint8x16x4_t val, const int lane);                                // ST4 {Vt.b - Vt4.b}[lane],[Xn]
void vst4q_lane_p8(poly8_t* ptr, poly8x16x4_t val, const int lane);                                // ST4 {Vt.b - Vt4.b}[lane],[Xn]
void vst4_lane_s64(int64_t* ptr, int64x1x4_t val, const int lane);                                 // ST4 {Vt.d - Vt4.d}[lane],[Xn]
void vst4q_lane_s64(int64_t* ptr, int64x2x4_t val, const int lane);                                // ST4 {Vt.d - Vt4.d}[lane],[Xn]
void vst4_lane_u64(uint64_t* ptr, uint64x1x4_t val, const int lane);                               // ST4 {Vt.d - Vt4.d}[lane],[Xn]
void vst4q_lane_u64(uint64_t* ptr, uint64x2x4_t val, const int lane);                              // ST4 {Vt.d - Vt4.d}[lane],[Xn]
void vst4_lane_p64(poly64_t* ptr, poly64x1x4_t val, const int lane);                               // ST4 {Vt.d - Vt4.d}[lane],[Xn]
void vst4q_lane_p64(poly64_t* ptr, poly64x2x4_t val, const int lane);                              // ST4 {Vt.d - Vt4.d}[lane],[Xn]
void vst4_lane_f64(float64_t* ptr, float64x1x4_t val, const int lane);                             // ST4 {Vt.d - Vt4.d}[lane],[Xn]
void vst4q_lane_f64(float64_t* ptr, float64x2x4_t val, const int lane);                            // ST4 {Vt.d - Vt4.d}[lane],[Xn]
...
float32x4_t vextq_f32(float32x4_t a, float32x4_t b, const int n);                                  // EXT Vd.16B,Vn.16B,Vm.16B,#(n<<2)
float64x1_t vext_f64(float64x1_t a, float64x1_t b, const int n);                                   // EXT Vd.8B,Vn.8B,Vm.8B,#(n<<3)
float64x2_t vextq_f64(float64x2_t a, float64x2_t b, const int n);                                  // EXT Vd.16B,Vn.16B,Vm.16B,#(n<<3)
...
int8x8_t vqtbx2_s8(int8x8_t a, int8x16x2_t t, uint8x8_t idx);                                      // TBX Vd.8B,{Vn.16B - Vn+1.16B},Vm.8B
int8x16_t vqtbx2q_s8(int8x16_t a, int8x16x2_t t, uint8x16_t idx);                                  // TBX Vd.16B,{Vn.16B - Vn+1.16B},Vm.16B
uint8x8_t vqtbx2_u8(uint8x8_t a, uint8x16x2_t t, uint8x8_t idx);                                   // TBX Vd.8B,{Vn.16B - Vn+1.16B},Vm.8B
uint8x16_t vqtbx2q_u8(uint8x16_t a, uint8x16x2_t t, uint8x16_t idx);                               // TBX Vd.16B,{Vn.16B - Vn+1.16B},Vm.16B
poly8x8_t vqtbx2_p8(poly8x8_t a, poly8x16x2_t t, uint8x8_t idx);                                   // TBX Vd.8B,{Vn.16B - Vn+1.16B},Vm.8B
poly8x16_t vqtbx2q_p8(poly8x16_t a, poly8x16x2_t t, uint8x16_t idx);                               // TBX Vd.16B,{Vn.16B - Vn+1.16B},Vm.16B
int8x8_t vqtbx3_s8(int8x8_t a, int8x16x3_t t, uint8x8_t idx);                                      // TBX Vd.8B,{Vn.16B - Vn+2.16B},Vm.8B
int8x16_t vqtbx3q_s8(int8x16_t a, int8x16x3_t t, uint8x16_t idx);                                  // TBX Vd.16B,{Vn.16B - Vn+2.16B},Vm.16B
uint8x8_t vqtbx3_u8(uint8x8_t a, uint8x16x3_t t, uint8x8_t idx);                                   // TBX Vd.8B,{Vn.16B - Vn+2.16B},Vm.8B
uint8x16_t vqtbx3q_u8(uint8x16_t a, uint8x16x3_t t, uint8x16_t idx);                               // TBX Vd.16B,{Vn.16B - Vn+2.16B},Vm.16B
poly8x8_t vqtbx3_p8(poly8x8_t a, poly8x16x3_t t, uint8x8_t idx);                                   // TBX Vd.8B,{Vn.16B - Vn+2.16B},Vm.8B
poly8x16_t vqtbx3q_p8(poly8x16_t a, poly8x16x3_t t, uint8x16_t idx);                               // TBX Vd.16B,{Vn.16B - Vn+2.16B},Vm.16B
int8x8_t vqtbx4_s8(int8x8_t a, int8x16x4_t t, uint8x8_t idx);                                      // TBX Vd.8B,{Vn.16B - Vn+3.16B},Vm.8B
int8x16_t vqtbx4q_s8(int8x16_t a, int8x16x4_t t, uint8x16_t idx);                                  // TBX Vd.16B,{Vn.16B - Vn+3.16B},Vm.16B
uint8x8_t vqtbx4_u8(uint8x8_t a, uint8x16x4_t t, uint8x8_t idx);                                   // TBX Vd.8B,{Vn.16B - Vn+3.16B},Vm.8B
uint8x16_t vqtbx4q_u8(uint8x16_t a, uint8x16x4_t t, uint8x16_t idx);                               // TBX Vd.16B,{Vn.16B - Vn+3.16B},Vm.16B
poly8x8_t vqtbx4_p8(poly8x8_t a, poly8x16x4_t t, uint8x8_t idx);                                   // TBX Vd.8B,{Vn.16B - Vn+3.16B},Vm.8B
poly8x16_t vqtbx4q_p8(poly8x16_t a, poly8x16x4_t t, uint8x16_t idx);                               // TBX Vd.16B,{Vn.16B - Vn+3.16B},Vm.16B
...
uint32x4_t vsha1su0q_u32(uint32x4_t w0_3, uint32x4_t w4_7, uint32x4_t w8_11);                      // SHA1SU0 Vd.4S,Vn.4S,Vm.4S
uint32x4_t vsha1su1q_u32(uint32x4_t tw0_3, uint32x4_t w12_15);                                     // SHA1SU1 Vd.4S,Vn.4S
uint32x4_t vsha256hq_u32(uint32x4_t hash_abcd, uint32x4_t hash_efgh, uint32x4_t wk);               // SHA256H Qd,Qn,Vm.4S
uint32x4_t vsha256h2q_u32(uint32x4_t hash_efgh, uint32x4_t hash_abcd, uint32x4_t wk);              // SHA256H2 Qd,Qn,Vm.4S
uint32x4_t vsha256su0q_u32(uint32x4_t w0_3, uint32x4_t w4_7);                                      // SHA256SU0 Vd.4S,Vn.4S
uint32x4_t vsha256su1q_u32(uint32x4_t tw0_3, uint32x4_t w8_11, uint32x4_t w12_15);                 // SHA256SU1 Vd.4S,Vn.4S,Vm.4S
...
int16x4_t vqrdmlah_lane_s16(int16x4_t a, int16x4_t b, int16x4_t v, const int lane);                // SQRDMLAH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqrdmlahq_lane_s16(int16x8_t a, int16x8_t b, int16x4_t v, const int lane);               // SQRDMLAH Vd.8H,Vn.8H,Vm.H[lane]
int16x4_t vqrdmlah_laneq_s16(int16x4_t a, int16x4_t b, int16x8_t v, const int lane);               // SQRDMLAH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqrdmlahq_laneq_s16(int16x8_t a, int16x8_t b, int16x8_t v, const int lane);              // SQRDMLAH Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vqrdmlah_lane_s32(int32x2_t a, int32x2_t b, int32x2_t v, const int lane);                // SQRDMLAH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqrdmlahq_lane_s32(int32x4_t a, int32x4_t b, int32x2_t v, const int lane);               // SQRDMLAH Vd.4S,Vn.4S,Vm.S[lane]
int32x2_t vqrdmlah_laneq_s32(int32x2_t a, int32x2_t b, int32x4_t v, const int lane);               // SQRDMLAH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqrdmlahq_laneq_s32(int32x4_t a, int32x4_t b, int32x4_t v, const int lane);              // SQRDMLAH Vd.4S,Vn.4S,Vm.S[lane]
int16x4_t vqrdmlsh_lane_s16(int16x4_t a, int16x4_t b, int16x4_t v, const int lane);                // SQRDMLSH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqrdmlshq_lane_s16(int16x8_t a, int16x8_t b, int16x4_t v, const int lane);               // SQRDMLSH Vd.8H,Vn.8H,Vm.H[lane]
int16x4_t vqrdmlsh_laneq_s16(int16x4_t a, int16x4_t b, int16x8_t v, const int lane);               // SQRDMLSH Vd.4H,Vn.4H,Vm.H[lane]
int16x8_t vqrdmlshq_laneq_s16(int16x8_t a, int16x8_t b, int16x8_t v, const int lane);              // SQRDMLSH Vd.8H,Vn.8H,Vm.H[lane]
int32x2_t vqrdmlsh_lane_s32(int32x2_t a, int32x2_t b, int32x2_t v, const int lane);                // SQRDMLSH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqrdmlshq_lane_s32(int32x4_t a, int32x4_t b, int32x2_t v, const int lane);               // SQRDMLSH Vd.4S,Vn.4S,Vm.S[lane]
int32x2_t vqrdmlsh_laneq_s32(int32x2_t a, int32x2_t b, int32x4_t v, const int lane);               // SQRDMLSH Vd.2S,Vn.2S,Vm.S[lane]
int32x4_t vqrdmlshq_laneq_s32(int32x4_t a, int32x4_t b, int32x4_t v, const int lane);              // SQRDMLSH Vd.4S,Vn.4S,Vm.S[lane]
...
int16_t vqrdmlahh_lane_s16(int16_t a, int16_t b, int16x4_t v, const int lane);                     // SQRDMLAH Hd,Hn,Vm.H[lane]
int16_t vqrdmlahh_laneq_s16(int16_t a, int16_t b, int16x8_t v, const int lane);                    // SQRDMLAH Hd,Hn,Vm.H[lane]
int32_t vqrdmlahs_lane_s32(int32_t a, int32_t b, int32x2_t v, const int lane);                     // SQRDMLAH Sd,Sn,Vm.S[lane]
int32_t vqrdmlahs_laneq_s32(int32_t a, int32_t b, int32x4_t v, const int lane);                    // SQRDMLAH Sd,Sn,Vm.S[lane]
int16_t vqrdmlshh_lane_s16(int16_t a, int16_t b, int16x4_t v, const int lane);                     // SQRDMLSH Hd,Hn,Vm.H[lane]
int16_t vqrdmlshh_laneq_s16(int16_t a, int16_t b, int16x8_t v, const int lane);                    // SQRDMLSH Hd,Hn,Vm.H[lane]
int32_t vqrdmlshs_lane_s32(int32_t a, int32_t b, int32x2_t v, const int lane);                     // SQRDMLSH Sd,Sn,Vm.S[lane]
int32_t vqrdmlshs_laneq_s32(int32_t a, int32_t b, int32x4_t v, const int lane);                    // SQRDMLSH Sd,Sn,Vm.S[lane]
...
float16x4_t vfma_lane_f16(float16x4_t a, float16x4_t b, float16x4_t v, const int lane);            // FMLA Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vfmaq_lane_f16(float16x8_t a, float16x8_t b, float16x4_t v, const int lane);           // FMLA Vd.8H,Vn.8H,Vm.H[lane]
float16x4_t vfma_laneq_f16(float16x4_t a, float16x4_t b, float16x8_t v, const int lane);           // FMLA Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vfmaq_laneq_f16(float16x8_t a, float16x8_t b, float16x8_t v, const int lane);          // FMLA Vd.8H,Vn.8H,Vm.H[lane]
...
float16_t vfmah_lane_f16(float16_t a, float16_t b, float16x4_t v, const int lane);                 // FMLA Hd,Hn,Vm.H[lane]
float16_t vfmah_laneq_f16(float16_t a, float16_t b, float16x8_t v, const int lane);                // FMLA Hd,Hn,Vm.H[lane]
float16x4_t vfms_lane_f16(float16x4_t a, float16x4_t b, float16x4_t v, const int lane);            // FMLS Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vfmsq_lane_f16(float16x8_t a, float16x8_t b, float16x4_t v, const int lane);           // FMLS Vd.8H,Vn.8H,Vm.H[lane]
float16x4_t vfms_laneq_f16(float16x4_t a, float16x4_t b, float16x8_t v, const int lane);           // FMLS Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vfmsq_laneq_f16(float16x8_t a, float16x8_t b, float16x8_t v, const int lane);          // FMLS Vd.8H,Vn.8H,Vm.H[lane]
...
float16_t vfmsh_lane_f16(float16_t a, float16_t b, float16x4_t v, const int lane);                 // FMLS Hd,Hn,Vm.H[lane]
float16_t vfmsh_laneq_f16(float16_t a, float16_t b, float16x8_t v, const int lane);                // FMLS Hd,Hn,Vm.H[lane]
float16x4_t vmul_lane_f16(float16x4_t a, float16x4_t v, const int lane);                           // FMUL Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vmulq_lane_f16(float16x8_t a, float16x4_t v, const int lane);                          // FMUL Vd.8H,Vn.8H,Vm.H[lane]
float16x4_t vmul_laneq_f16(float16x4_t a, float16x8_t v, const int lane);                          // FMUL Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vmulq_laneq_f16(float16x8_t a, float16x8_t v, const int lane);                         // FMUL Vd.8H,Vn.8H,Vm.H[lane]
...
float16x4_t vmulx_lane_f16(float16x4_t a, float16x4_t v, const int lane);                          // FMULX Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vmulxq_lane_f16(float16x8_t a, float16x4_t v, const int lane);                         // FMULX Vd.8H,Vn.8H,Vm.H[lane]
float16x4_t vmulx_laneq_f16(float16x4_t a, float16x8_t v, const int lane);                         // FMULX Vd.4H,Vn.4H,Vm.H[lane]
float16x8_t vmulxq_laneq_f16(float16x8_t a, float16x8_t v, const int lane);                        // FMULX Vd.8H,Vn.8H,Vm.H[lane]
...
float16x8_t vextq_f16(float16x8_t a, float16x8_t b, const int n);                                  // EXT Vd.16B,Vn.16B,Vm.16B,#(n<<1)
...
uint32x2_t vdot_lane_u32(uint32x2_t r, uint8x8_t a, uint8x8_t b, const int lane);                  // UDOT Vd.2S,Vn.8B,Vm.4B[lane]
int32x2_t vdot_lane_s32(int32x2_t r, int8x8_t a, int8x8_t b, const int lane);                      // SDOT Vd.2S,Vn.8B,Vm.4B[lane]
uint32x4_t vdotq_laneq_u32(uint32x4_t r, uint8x16_t a, uint8x16_t b, const int lane);              // UDOT Vd.4S,Vn.16B,Vm.4B[lane]
int32x4_t vdotq_laneq_s32(int32x4_t r, int8x16_t a, int8x16_t b, const int lane);                  // SDOT Vd.4S,Vn.16B,Vm.4B[lane]
uint32x2_t vdot_laneq_u32(uint32x2_t r, uint8x8_t a, uint8x16_t b, const int lane);                // UDOT Vd.2S,Vn.8B,Vm.4B[lane]
int32x2_t vdot_laneq_s32(int32x2_t r, int8x8_t a, int8x16_t b, const int lane);                    // SDOT Vd.2S,Vn.8B,Vm.4B[lane]
uint32x4_t vdotq_lane_u32(uint32x4_t r, uint8x16_t a, uint8x8_t b, const int lane);                // UDOT Vd.4S,Vn.16B,Vm.4B[lane]
int32x4_t vdotq_lane_s32(int32x4_t r, int8x16_t a, int8x8_t b, const int lane);                    // SDOT Vd.4S,Vn.16B,Vm.4B[lane]
uint64x2_t vsha512hq_u64(uint64x2_t hash_ed, uint64x2_t hash_gf, uint64x2_t kwh_kwh2);             // SHA512H Qd,Qn,Vm.2D
uint64x2_t vsha512h2q_u64(uint64x2_t sum_ab, uint64x2_t hash_c_, uint64x2_t hash_ab);              // SHA512H2 Qd,Qn,Vm.2D
uint64x2_t vsha512su0q_u64(uint64x2_t w0_1, uint64x2_t w2_);                                       // SHA512SU0 Vd.2D,Vn.2D
uint64x2_t vsha512su1q_u64(uint64x2_t s01_s02, uint64x2_t w14_15, uint64x2_t w9_10);               // SHA512SU1 Vd.2D,Vn.2D,Vm.2D
...
uint32x4_t vsm3ss1q_u32(uint32x4_t a, uint32x4_t b, uint32x4_t c);                                 // SM3SS1 Vd.4S,Vn.4S,Vm.4S,Va.4S
uint32x4_t vsm3tt1aq_u32(uint32x4_t a, uint32x4_t b, uint32x4_t c, const int imm2);                // SM3TT1A Vd.4S,Vn.4S,Vm.4S[imm2]
uint32x4_t vsm3tt1bq_u32(uint32x4_t a, uint32x4_t b, uint32x4_t c, const int imm2);                // SM3TT1B Vd.4S,Vn.4S,Vm.4S[imm2]
uint32x4_t vsm3tt2aq_u32(uint32x4_t a, uint32x4_t b, uint32x4_t c, const int imm2);                // SM3TT2A Vd.4S,Vn.4S,Vm.4S[imm2]
uint32x4_t vsm3tt2bq_u32(uint32x4_t a, uint32x4_t b, uint32x4_t c, const int imm2);                // SM3TT2B Vd.4S,Vn.4S,Vm.4S[imm2]
uint32x4_t vsm3partw1q_u32(uint32x4_t a, uint32x4_t b, uint32x4_t c);                              // SM3PARTW1 Vd.4S,Vn.4S,Vm.4S
uint32x4_t vsm3partw2q_u32(uint32x4_t a, uint32x4_t b, uint32x4_t c);                              // SM3PARTW2 Vd.4S,Vn.4S,Vm.4S
...
float32x4_t vfmlalq_low_f16(float32x4_t r, float16x8_t a, float16x8_t b);                          // FMLAL Vd.4S,Vn.4H,Vm.4H
float32x4_t vfmlslq_low_f16(float32x4_t r, float16x8_t a, float16x8_t b);                          // FMLSL Vd.4S,Vn.4H,Vm.4H
float32x2_t vfmlal_high_f16(float32x2_t r, float16x4_t a, float16x4_t b);                          // FMLAL2 Vd.2S,Vn.2H,Vm.2H
float32x2_t vfmlsl_high_f16(float32x2_t r, float16x4_t a, float16x4_t b);                          // FMLSL2 Vd.2S,Vn.2H,Vm.2H
float32x4_t vfmlalq_high_f16(float32x4_t r, float16x8_t a, float16x8_t b);                         // FMLAL2 Vd.4S,Vn.4H,Vm.4H
float32x4_t vfmlslq_high_f16(float32x4_t r, float16x8_t a, float16x8_t b);                         // FMLSL2 Vd.4S,Vn.4H,Vm.4H
float32x2_t vfmlal_lane_low_f16(float32x2_t r, float16x4_t a, float16x4_t b, const int lane);      // FMLAL Vd.2S,Vn.2H,Vm.H[lane]
float32x2_t vfmlal_laneq_low_f16(float32x2_t r, float16x4_t a, float16x8_t b, const int lane);     // FMLAL Vd.2S,Vn.2H,Vm.H[lane]
float32x4_t vfmlalq_lane_low_f16(float32x4_t r, float16x8_t a, float16x4_t b, const int lane);     // FMLAL Vd.4S,Vn.4H,Vm.H[lane]
float32x4_t vfmlalq_laneq_low_f16(float32x4_t r, float16x8_t a, float16x8_t b, const int lane);    // FMLAL Vd.4S,Vn.4H,Vm.H[lane]
float32x2_t vfmlsl_lane_low_f16(float32x2_t r, float16x4_t a, float16x4_t b, const int lane);      // FMLSL Vd.2S,Vn.2H,Vm.H[lane]
float32x2_t vfmlsl_laneq_low_f16(float32x2_t r, float16x4_t a, float16x8_t b, const int lane);     // FMLSL Vd.2S,Vn.2H,Vm.H[lane]
float32x4_t vfmlslq_lane_low_f16(float32x4_t r, float16x8_t a, float16x4_t b, const int lane);     // FMLSL Vd.4S,Vn.4H,Vm.H[lane]
float32x4_t vfmlslq_laneq_low_f16(float32x4_t r, float16x8_t a, float16x8_t b, const int lane);    // FMLSL Vd.4S,Vn.4H,Vm.H[lane]
float32x2_t vfmlal_lane_high_f16(float32x2_t r, float16x4_t a, float16x4_t b, const int lane);     // FMLAL2 Vd.2S,Vn.2H,Vm.H[lane]
float32x2_t vfmlsl_lane_high_f16(float32x2_t r, float16x4_t a, float16x4_t b, const int lane);     // FMLSL2 Vd.2S,Vn.2H,Vm.H[lane]
float32x4_t vfmlalq_lane_high_f16(float32x4_t r, float16x8_t a, float16x4_t b, const int lane);    // FMLAL2 Vd.4S,Vn.4H,Vm.H[lane]
float32x4_t vfmlslq_lane_high_f16(float32x4_t r, float16x8_t a, float16x4_t b, const int lane);    // FMLSL2 Vd.4S,Vn.4H,Vm.H[lane]
float32x2_t vfmlal_laneq_high_f16(float32x2_t r, float16x4_t a, float16x8_t b, const int lane);    // FMLAL2 Vd.2S,Vn.2H,Vm.H[lane]
float32x2_t vfmlsl_laneq_high_f16(float32x2_t r, float16x4_t a, float16x8_t b, const int lane);    // FMLSL2 Vd.2S,Vn.2H,Vm.H[lane]
float32x4_t vfmlalq_laneq_high_f16(float32x4_t r, float16x8_t a, float16x8_t b, const int lane);   // FMLAL2 Vd.4S,Vn.4H,Vm.H[lane]
float32x4_t vfmlslq_laneq_high_f16(float32x4_t r, float16x8_t a, float16x8_t b, const int lane);   // FMLSL2 Vd.4S,Vn.4H,Vm.H[lane]
...
float16x8_t vcmlaq_lane_f16(float16x8_t r, float16x8_t a, float16x4_t b, const int lane);          // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#0
float32x4_t vcmlaq_lane_f32(float32x4_t r, float32x4_t a, float32x2_t b, const int lane);          // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#0
float16x8_t vcmlaq_laneq_f16(float16x8_t r, float16x8_t a, float16x8_t b, const int lane);         // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#0
float32x4_t vcmlaq_laneq_f32(float32x4_t r, float32x4_t a, float32x4_t b, const int lane);         // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#0
float16x4_t vcmla_rot90_f16(float16x4_t r, float16x4_t a, float16x4_t b);                          // FCMLA Vd.4H,Vn.4H,Vm.4H,#90
float32x2_t vcmla_rot90_f32(float32x2_t r, float32x2_t a, float32x2_t b);                          // FCMLA Vd.2S,Vn.2S,Vm.2S,#90
...
float16x8_t vcmlaq_rot90_lane_f16(float16x8_t r, float16x8_t a, float16x4_t b, const int lane);    // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#90
float32x4_t vcmlaq_rot90_lane_f32(float32x4_t r, float32x4_t a, float32x2_t b, const int lane);    // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#90
float16x8_t vcmlaq_rot90_laneq_f16(float16x8_t r, float16x8_t a, float16x8_t b, const int lane);   // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#90
float32x4_t vcmlaq_rot90_laneq_f32(float32x4_t r, float32x4_t a, float32x4_t b, const int lane);   // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#90
float16x4_t vcmla_rot180_f16(float16x4_t r, float16x4_t a, float16x4_t b);                         // FCMLA Vd.4H,Vn.4H,Vm.4H,#180
float32x2_t vcmla_rot180_f32(float32x2_t r, float32x2_t a, float32x2_t b);                         // FCMLA Vd.2S,Vn.2S,Vm.2S,#180
...
float16x8_t vcmlaq_rot180_lane_f16(float16x8_t r, float16x8_t a, float16x4_t b, const int lane);   // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#180
float32x4_t vcmlaq_rot180_lane_f32(float32x4_t r, float32x4_t a, float32x2_t b, const int lane);   // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#180
float16x8_t vcmlaq_rot180_laneq_f16(float16x8_t r, float16x8_t a, float16x8_t b, const int lane);  // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#180
float32x4_t vcmlaq_rot180_laneq_f32(float32x4_t r, float32x4_t a, float32x4_t b, const int lane);  // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#180
float16x4_t vcmla_rot270_f16(float16x4_t r, float16x4_t a, float16x4_t b);                         // FCMLA Vd.4H,Vn.4H,Vm.4H,#270
float32x2_t vcmla_rot270_f32(float32x2_t r, float32x2_t a, float32x2_t b);                         // FCMLA Vd.2S,Vn.2S,Vm.2S,#270
...
float16x8_t vcmlaq_rot270_lane_f16(float16x8_t r, float16x8_t a, float16x4_t b, const int lane);   // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#270
float32x4_t vcmlaq_rot270_lane_f32(float32x4_t r, float32x4_t a, float32x2_t b, const int lane);   // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#270
float16x8_t vcmlaq_rot270_laneq_f16(float16x8_t r, float16x8_t a, float16x8_t b, const int lane);  // FCMLA Vd.8H,Vn.8H,Vm.H[lane],#270
float32x4_t vcmlaq_rot270_laneq_f32(float32x4_t r, float32x4_t a, float32x4_t b, const int lane);  // FCMLA Vd.4S,Vn.4S,Vm.S[lane],#270
...
int32x2_t vusdot_lane_s32(int32x2_t r, uint8x8_t a, int8x8_t b, const int lane);                   // USDOT Vd.2S,Vn.8B,Vm.4B[lane]
int32x2_t vsudot_lane_s32(int32x2_t r, int8x8_t a, uint8x8_t b, const int lane);                   // SUDOT Vd.2S,Vn.8B,Vm.4B[lane]
int32x2_t vusdot_laneq_s32(int32x2_t r, uint8x8_t a, int8x16_t b, const int lane);                 // USDOT Vd.2S,Vn.8B,Vm.4B[lane]
int32x2_t vsudot_laneq_s32(int32x2_t r, int8x8_t a, uint8x16_t b, const int lane);                 // SUDOT Vd.2S,Vn.8B,Vm.4B[lane]
int32x4_t vusdotq_s32(int32x4_t r, uint8x16_t a, int8x16_t b);                                     // USDOT Vd.4S,Vn.16B,Vm.16B
int32x4_t vusdotq_lane_s32(int32x4_t r, uint8x16_t a, int8x8_t b, const int lane);                 // USDOT Vd.4S,Vn.16B,Vm.4B[lane]
int32x4_t vsudotq_lane_s32(int32x4_t r, int8x16_t a, uint8x8_t b, const int lane);                 // SUDOT Vd.4S,Vn.16B,Vm.4B[lane]
int32x4_t vusdotq_laneq_s32(int32x4_t r, uint8x16_t a, int8x16_t b, const int lane);               // USDOT Vd.4S,Vn.16B,Vm.4B[lane]
int32x4_t vsudotq_laneq_s32(int32x4_t r, int8x16_t a, uint8x16_t b, const int lane);               // SUDOT Vd.4S,Vn.16B,Vm.4B[lane]
...
bfloat16x4_t vset_lane_bf16(bfloat16_t a, bfloat16x4_t v, const int lane);                         // INS Vd.H[lane],Vn.H[0]
bfloat16x8_t vsetq_lane_bf16(bfloat16_t a, bfloat16x8_t v, const int lane);                        // INS Vd.H[lane],Vn.H[0]
...
bfloat16x4_t vld1_lane_bf16(bfloat16_t const* ptr, bfloat16x4_t src, const int lane);              // LD1 {Vt.H}[lane],[Xn]
bfloat16x8_t vld1q_lane_bf16(bfloat16_t const* ptr, bfloat16x8_t src, const int lane);             // LD1 {Vt.H}[lane],[Xn]
...
bfloat16x4x2_t vld2_lane_bf16(bfloat16_t const* ptr, bfloat16x4x2_t src, const int lane);          // LD2 {Vt.h - Vt2.h}[lane],[Xn]
bfloat16x8x2_t vld2q_lane_bf16(bfloat16_t const* ptr, bfloat16x8x2_t src, const int lane);         // LD2 {Vt.h - Vt2.h}[lane],[Xn]
bfloat16x4x3_t vld3_lane_bf16(bfloat16_t const* ptr, bfloat16x4x3_t src, const int lane);          // LD3 {Vt.h - Vt3.h}[lane],[Xn]
bfloat16x8x3_t vld3q_lane_bf16(bfloat16_t const* ptr, bfloat16x8x3_t src, const int lane);         // LD3 {Vt.h - Vt3.h}[lane],[Xn]
bfloat16x4x4_t vld4_lane_bf16(bfloat16_t const* ptr, bfloat16x4x4_t src, const int lane);          // LD4 {Vt.h - Vt4.h}[lane],[Xn]
bfloat16x8x4_t vld4q_lane_bf16(bfloat16_t const* ptr, bfloat16x8x4_t src, const int lane);         // LD4 {Vt.h - Vt4.h}[lane],[Xn]
void vst2_lane_bf16(bfloat16_t* ptr, bfloat16x4x2_t val, const int lane);                          // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst2q_lane_bf16(bfloat16_t* ptr, bfloat16x8x2_t val, const int lane);                         // ST2 {Vt.h - Vt2.h}[lane],[Xn]
void vst3_lane_bf16(bfloat16_t* ptr, bfloat16x4x3_t val, const int lane);                          // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst3q_lane_bf16(bfloat16_t* ptr, bfloat16x8x3_t val, const int lane);                         // ST3 {Vt.h - Vt3.h}[lane],[Xn]
void vst4_lane_bf16(bfloat16_t* ptr, bfloat16x4x4_t val, const int lane);                          // ST4 {Vt.h - Vt4.h}[lane],[Xn]
void vst4q_lane_bf16(bfloat16_t* ptr, bfloat16x8x4_t val, const int lane);                         // ST4 {Vt.h - Vt4.h}[lane],[Xn]
...
bfloat16x4_t vcopy_lane_bf16(bfloat16x4_t a, const int lane1, bfloat16x4_t b, const int lane2);    // INS Vd.H[lane1],Vn.H[lane2]
bfloat16x8_t vcopyq_lane_bf16(bfloat16x8_t a, const int lane1, bfloat16x4_t b, const int lane2);   // INS Vd.H[lane1],Vn.H[lane2]
bfloat16x4_t vcopy_laneq_bf16(bfloat16x4_t a, const int lane1, bfloat16x8_t b, const int lane2);   // INS Vd.H[lane1],Vn.H[lane2]
bfloat16x8_t vcopyq_laneq_bf16(bfloat16x8_t a, const int lane1, bfloat16x8_t b, const int lane2);  // INS Vd.H[lane1],Vn.H[lane2]
...
float32x2_t vbfdot_lane_f32(float32x2_t r, bfloat16x4_t a, bfloat16x4_t b, const int lane);        // BFDOT Vd.2S,Vn.4H,Vm.2H[lane]
float32x4_t vbfdotq_laneq_f32(float32x4_t r, bfloat16x8_t a, bfloat16x8_t b, const int lane);      // BFDOT Vd.4S,Vn.8H,Vm.2H[lane]
float32x2_t vbfdot_laneq_f32(float32x2_t r, bfloat16x4_t a, bfloat16x8_t b, const int lane);       // BFDOT Vd.2S,Vn.4H,Vm.2H[lane]
float32x4_t vbfdotq_lane_f32(float32x4_t r, bfloat16x8_t a, bfloat16x4_t b, const int lane);       // BFDOT Vd.4S,Vn.8H,Vm.2H[lane]
float32x4_t vbfmmlaq_f32(float32x4_t r, bfloat16x8_t a, bfloat16x8_t b);                           // BFMMLA Vd.4S,Vn.8H,Vm.8H
float32x4_t vbfmlalbq_f32(float32x4_t r, bfloat16x8_t a, bfloat16x8_t b);                          // BFMLALB Vd.4S,Vn.8H,Vm.8H
float32x4_t vbfmlaltq_f32(float32x4_t r, bfloat16x8_t a, bfloat16x8_t b);                          // BFMLALT Vd.4S,Vn.8H,Vm.8H
float32x4_t vbfmlalbq_lane_f32(float32x4_t r, bfloat16x8_t a, bfloat16x4_t b, const int lane);     // BFMLALB Vd.4S,Vn.8H,Vm.H[lane]
float32x4_t vbfmlalbq_laneq_f32(float32x4_t r, bfloat16x8_t a, bfloat16x8_t b, const int lane);    // BFMLALB Vd.4S,Vn.8H,Vm.H[lane]
float32x4_t vbfmlaltq_lane_f32(float32x4_t r, bfloat16x8_t a, bfloat16x4_t b, const int lane);     // BFMLALT Vd.4S,Vn.8H,Vm.H[lane]
float32x4_t vbfmlaltq_laneq_f32(float32x4_t r, bfloat16x8_t a, bfloat16x8_t b, const int lane);    // BFMLALT Vd.4S,Vn.8H,Vm.H[lane]
