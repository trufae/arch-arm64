...
class Arm64Architecture : public Architecture
{
 ...
 public:
	Arm64Architecture() :
	    Architecture("aarch64"), m_bits(64)
	{
	}
	...
	bool Assemble(const string& code, uint64_t addr, DataBuffer& result, string& errors) override
	{
		...
		errors.clear();
		assembleResult = BNLlvmServicesAssemble(code.c_str(), LLVM_SVCS_DIALECT_UNSPEC, "aarch64-none-none", LLVM_SVCS_CM_DEFAULT, LLVM_SVCS_RM_STATIC, &instrBytes, &instrBytesLen, &err, &errLen);
	...
	virtual size_t GetFlagWriteLowLevelIL(BNLowLevelILOperation op, size_t size, uint32_t flagWriteType, uint32_t flag, BNRegisterOrConstant* operands, size_t operandCount, LowLevelILFunction& il) override
	{
		switch (op)
		{
		case LLIL_AND:
...
{
 public:
	Arm64CallingConvention(Architecture* arch) :
	    CallingConvention(arch, "cdecl")
	{
	}


	Arm64CallingConvention(Architecture* arch, const string& name) :
	    CallingConvention(arch, name)
	{
	}
	...
	{
		return vector<uint32_t> {REG_X0, REG_X1, REG_X2, REG_X3, REG_X4, REG_X5, REG_X6, REG_X7, REG_X8, REG_X9, REG_X10, REG_X11, REG_X12, REG_X13, REG_X14, REG_X15, REG_X16, REG_X17, REG_X18, REG_X30, REG_Q0, REG_Q1, REG_Q2, REG_Q3, REG_Q4, REG_Q5, REG_Q6, REG_Q7, REG_Q16, REG_Q17, REG_Q18, REG_Q19, REG_Q20, REG_Q21, REG_Q22, REG_Q23, REG_Q24, REG_Q25, REG_Q26, REG_Q27, REG_Q28, REG_Q29, REG_Q30, REG_Q31};
	}


	virtual vector<uint32_t> GetCalleeSavedRegisters() override
	{
		return vector<uint32_t> {REG_X19, REG_X20, REG_X21, REG_X22, REG_X23, REG_X24, REG_X25, REG_X26, REG_X27, REG_X28, REG_X29};
	}
...
{
 public:
	AppleArm64CallingConvention(Architecture* arch) :
	    Arm64CallingConvention(arch, "apple-arm64")
	{
	}
...
{
 public:
	LinuxArm64SystemCallConvention(Architecture* arch) :
	    CallingConvention(arch, "linux-syscall")
	{
	}
...
{
 public:
	WindowsArm64SystemCallConvention(Architecture* arch) :
	    CallingConvention(arch, "windows-syscall")
	{
	}
...
{
 public:
	MacosArm64SystemCallConvention(Architecture* arch) :
	    CallingConvention(arch, "macos-syscall")
	{
	}
