...
def normalize(instxt):
	...
	for dec_imm in re.findall(r'#\d+$', instxt):
		if not instxt.endswith(dec_imm):
			continue
...
def compare_disassembly_token(a, b):
	...
	if a.startswith('x') and a.startswith(b) and a[len(b):] == '.d':
		return 0
	if b.startswith('x') and b.startswith(a) and b[len(a):] == '.d':
		return 0
	if a == 'sp' and b == 'sp.d':
		return 0
	if a == 'sp.d' and b == 'sp':
	...
	if (a, b) == ('cs', 'hs') or (a, b) == ('hs', 'cs'):
		return 0
	# cc is equivalent to lo (carry clear vs. lower)
	if (a, b) == ('cc', 'lo') or (a, b) == ('lo', 'cc'):
		return 0

	#print('after trash removal:')
	#print('a: ', a)
	#print('b: ', b)
	if a[0] in trash or b[0] in trash:
		return -1
	if a[-1] in trash or b[-1] in trash:
		return -1
	try:
		# 0xff == 255
		if '0x' in a:
		...
		if a.startswith('0x') and len(a) == 4 and b.startswith('-'):
			if struct.unpack('<b', struct.pack('<B', a_val))[0] == b_val:
				return 0
		if b.startswith('0x') and len(b) == 4 and a.startswith('-'):
			if struct.unpack('<b', struct.pack('<B', b_val))[0] == a_val:
				return 0
		# 0xfffffffe == -2
		if a.startswith('0x') and len(a) == 10 and b.startswith('-'):
			if struct.unpack('<i', struct.pack('<I', a_val))[0] == b_val:
				return 0
		if b.startswith('0x') and len(b) == 10 and a.startswith('-'):
			if struct.unpack('<i', struct.pack('<I', b_val))[0] == a_val:
				return 0
		# 0xffffffffffffffff == -2
		if a.startswith('0x') and len(a) == 18 and b.startswith('-'):
			if struct.unpack('<q', struct.pack('<Q', a_val))[0] == b_val:
				return 0
		if b.startswith('0x') and len(b) == 18 and a.startswith('-'):
			if struct.unpack('<q', struct.pack('<Q', b_val))[0] == a_val:
				return 0
...
def excusable_difference(actual, expected):
	if actual == 'dgh' and expected.startswith('hint'):
		return True
	if actual == 'cfinv' and expected.startswith('msr'):
		return True
	if actual.startswith('mov') and expected.startswith('dupm'):
		return True  # spec is screwed up here
	if actual == 'sb' and expected.startswith('msr'):
		return True
	if actual == 'xaflag' and expected.startswith('msr'):
		return True
	if actual.startswith('at ') and expected.startswith('sys'):
		return True
	if actual.startswith('dc ') and expected.startswith('sys'):
		return True
	if actual.startswith('cfp ') and expected.startswith('sys'):
		return True
	if actual.startswith('cmpp ') and expected.startswith('subps '):
		return True
	if actual.startswith('tlbi') and expected.startswith('sys '):
		return True
	if actual.startswith('msr ssbs') and expected.startswith('msr s0_'):
		return True
	if actual.startswith('msr pan') and expected.startswith('msr s0_'):
		return True
	if actual.startswith('axflag'):
		return True
