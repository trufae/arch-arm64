...
const char* get_register_arrspec(Register reg, const InstructionOperand* operand)
{
	...
	if (operand->laneUsed || is_sve || is_pred)
	{
		...
		case ARRSPEC_2HALVES:
			return ".h";
		case ARRSPEC_4BYTES:
			return ".4b";  // not an error, UDOT_asimdelem_D and SDOT_asimdelem_D use this
		case ARRSPEC_1HALF:
			return ".h";
		case ARRSPEC_1BYTE:
			return ".b";
    ...
    uint32_t outBufferSize)
{
	...
	case MEM_EXTENDED:
		...
		if (operand->shiftType != ShiftType_NONE)
		{
			if (snprintf(extendBuff, sizeof(extendBuff), ", %s%s", get_shift(operand->shiftType), immBuff) >= sizeof(extendBuff))
			{
				return FAILED_TO_DISASSEMBLE_OPERAND;
			}
		}
...
uint32_t get_register(const InstructionOperand* operand, uint32_t registerNumber, char* outBuffer, uint32_t outBufferSize)
{
	...
	{
		if (snprintf(outBuffer, outBufferSize, "%s", get_system_register_name(operand->sysreg)) >= outBufferSize)
			return FAILED_TO_DISASSEMBLE_REGISTER;
		return 0;
	}
...
uint32_t get_multireg_operand(const InstructionOperand* operand, char* result, uint32_t result_sz)
{
	...
	{
	case 1:
		rc = snprintf(result, result_sz, "{%s}%s", reg_str[0], lane_str);
		break;
	case 2:
		rc = snprintf(result, result_sz, "{%s, %s}%s", reg_str[0], reg_str[1], lane_str);
		break;
	case 3:
		rc = snprintf(result, result_sz, "{%s, %s, %s}%s", reg_str[0], reg_str[1], reg_str[2], lane_str);
		break;
	case 4:
		rc = snprintf(result, result_sz, "{%s, %s, %s, %s}%s", reg_str[0], reg_str[1], reg_str[2], reg_str[3], lane_str);
		break;
	default:
		return FAILED_TO_DISASSEMBLE_OPERAND;
	}
...
uint32_t get_shifted_immediate(const InstructionOperand* instructionOperand, char* outBuffer, uint32_t outBufferSize, uint32_t type)
{
	...
	{
		if (snprintf(outBuffer, outBufferSize, "#%s%#" PRIx64 "%s", sign, imm, shiftBuff) >= outBufferSize)
			return FAILED_TO_DISASSEMBLE_OPERAND;
	}
	return DISASM_SUCCESS;
}
...
int aarch64_disassemble(Instruction* instruction, char* buf, size_t buf_sz)
{
	...
	for (int i = 0; i < MAX_OPERANDS && instruction->operands[i].operandClass != NONE; i++)
	{
		...
		case MEM_POST_IDX:
			...
			break;
		case CONDITION:
			if (snprintf(tmpOperandString, sizeof(tmpOperandString), "%s", get_condition((Condition)instruction->operands[i].cond)) >= sizeof(tmpOperandString))
				return FAILED_TO_DISASSEMBLE_OPERAND;
			operand = tmpOperandString;
			break;
		case NAME:
	...
	memset(buf, 0, buf_sz);
	if (snprintf(buf, buf_sz, "%s%s%s%s%s%s", get_operation(instruction), operandStrings[0], operandStrings[1], operandStrings[2], operandStrings[3], operandStrings[4]) >= buf_sz)
