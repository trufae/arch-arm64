...
bool NeonGetLowLevelILForInstruction(Architecture* arch, uint64_t addr, LowLevelILFunction& il, Instruction& instr, size_t addrSize)
{
	...
	case ENC_DUP_ASIMDINS_DR_R:
		if (instr.operands[1].arrSpec == ARRSPEC_8BYTES)
			intrin_id = ARM64_INTRIN_VDUP_N_S8;  // DUP Vd.8B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_16BYTES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_S8;  // DUP Vd.16B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VDUP_N_S16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_S16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2SINGLES)
			intrin_id = ARM64_INTRIN_VDUP_N_S32;  // DUP Vd.2S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4SINGLES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_S32;  // DUP Vd.4S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2DOUBLES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_S64;  // DUP Vd.2D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8BYTES)
			intrin_id = ARM64_INTRIN_VDUP_N_U8;  // DUP Vd.8B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_16BYTES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_U8;  // DUP Vd.16B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VDUP_N_U16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_U16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2SINGLES)
			intrin_id = ARM64_INTRIN_VDUP_N_U32;  // DUP Vd.2S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4SINGLES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_U32;  // DUP Vd.4S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2DOUBLES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_U64;  // DUP Vd.2D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2DOUBLES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_P64;  // DUP Vd.2D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2SINGLES)
			intrin_id = ARM64_INTRIN_VDUP_N_F32;  // DUP Vd.2S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4SINGLES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_F32;  // DUP Vd.4S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8BYTES)
			intrin_id = ARM64_INTRIN_VDUP_N_P8;  // DUP Vd.8B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_16BYTES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_P8;  // DUP Vd.16B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VDUP_N_P16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_P16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2DOUBLES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_F64;  // DUP Vd.2D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8BYTES)
			intrin_id = ARM64_INTRIN_VMOV_N_S8;  // DUP Vd.8B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_16BYTES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_S8;  // DUP Vd.16B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VMOV_N_S16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_S16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2SINGLES)
			intrin_id = ARM64_INTRIN_VMOV_N_S32;  // DUP Vd.2S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4SINGLES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_S32;  // DUP Vd.4S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VMOV_N_S64;  // DUP Vd.1D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2DOUBLES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_S64;  // DUP Vd.2D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8BYTES)
		...
		if (instr.operands[1].arrSpec == ARRSPEC_2SINGLES)
			intrin_id = ARM64_INTRIN_VMOV_N_F32;  // DUP Vd.2S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4SINGLES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_F32;  // DUP Vd.4S,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8BYTES)
			intrin_id = ARM64_INTRIN_VMOV_N_P8;  // DUP Vd.8B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_16BYTES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_P8;  // DUP Vd.16B,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VMOV_N_P16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_P16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VMOV_N_F64;  // DUP Vd.1D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_2DOUBLES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_F64;  // DUP Vd.2D,rn
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_S8;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_S16;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_S32;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_S64;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_U8;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_U16;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_U32;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_U64;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_P64;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_F16;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_F32;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_P8;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_P16;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_F64;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_S8;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_S16;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_S32;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_S64;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_U8;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_U16;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_U32;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_U64;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_P64;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_F16;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_F32;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_P8;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_P16;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_F64;  // DUP Vd.1D,Vn.D[0]
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VMOV_N_F16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VMOVQ_N_F16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VDUP_N_F16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_F16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_4HALVES)
			intrin_id = ARM64_INTRIN_VDUP_N_BF16;  // DUP Vd.4H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_8HALVES)
			intrin_id = ARM64_INTRIN_VDUPQ_N_BF16;  // DUP Vd.8H,rn
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_HIGH_BF16;  // DUP Vd.1D,Vn.D[1]
		if (instr.operands[1].arrSpec == ARRSPEC_1DOUBLE)
			intrin_id = ARM64_INTRIN_VGET_LOW_BF16;  // DUP Vd.1D,Vn.D[0]
		add_input_reg(inputs, il, instr.operands[1]);
		add_output_reg(outputs, il, instr.operands[0]);
		break;
